...
const ANNOUNCEMENTS = [
  { id: "a1", title: "7월 전사 워크샵 안내", date: "2026-07-10", body: "이번 달 워크샵은 주민센터 대회의실에서 진행됩니다. 각 팀 리드는 회의실 예약 후 어젠다를 게시판에 공유해 주세요." },
  ...
  { id: "a4", title: "몰입의 방 이용 수칙", date: "2026-07-20", body: "몰입의 방은 자유 허들 공간입니다. 수면의 방은 '방해 금지' 원칙을 지켜주세요." },
...
const CAL_EVENTS = {
  ...
  "2026-07-28": ["월말 결산 시작"],
  "2026-07-31": ["🎉 월말 결산 마감", "치앙마이 워크샵 출발"],
};
...
/* 주민센터 기준 4방향 표지판 */
function GuidePost() {
  const Plank = ({ dir, text, color }) => (
    <div style={{ display: "flex", alignItems: "center", gap: 4, background: color, color: C.white, border: `2px solid ${C.ink}`, padding: "2px 6px", fontSize: 10, fontWeight: "bold", whiteSpace: "nowrap", boxShadow: `0 2px 0 ${C.woodDark}` }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
      <Plank dir="up" text="위 · 업무(대형건물)" color={C.bankRoof} />
      <div style={{ display: "flex", gap: 24 }}>
        <Plank dir="left" text="왼쪽 · 은행/개인집" color={C.wood} />
        <Plank dir="right" text="오른쪽 · 사이드/치앙마이" color={C.good} />
      </div>
      <Plank dir="down" text="아래 · 수영장/헬스장" color={C.danger} />
      <div style={{ width: 6, height: 42, background: C.woodDark, border: `1px solid ${C.ink}` }} />
    </div>
  );
}
...
function RoomView({ title, icon, sub, bg, roomW = 640, roomH = 400, furniture, start, onBack, paused = false, children, headerBg = C.parch, banner = null }) {
...
function buildWorld() {
  ...
  // 대형건물(상단)
  const bigPos = { app: [960, 320], underwear: [1250, 270], socks: [1560, 300], alba: [1330, 500] };
  ...
  // 소형건물(우측 클러스터)
  const smalls = [
    { id: "thanks", label: "🙏 감사의 방", tint: "#e0a13d", x: 1770, y: 580 },
    { id: "heart", label: "💌 마음의 방", tint: "#d76b96", x: 1985, y: 690 },
    { id: "listening", label: "🎵 리스닝 방", tint: "#5b8def", x: 1760, y: 850 },
    { id: "reels", label: "📱 릴스방", tint: "#3fa07a", x: 1985, y: 960 },
    { id: "smoke", label: "🚬 흡연의 방", tint: "#7a8b99", x: 1800, y: 1130 },
  ];
...
function WorldView({ pos, setPos, day, gems, rentedHouses, onEnter, onNextDay }) {
  const [facing, setFacing] = useState(1);
  const [moving, setMoving] = useState(false);
  const [near, setNear] = useState(null);
  ...
  return (
    <Panel style={{ padding: 0, overflow: "hidden" }}>
      <div style={{ padding: "10px 14px", borderBottom: `3px solid ${C.parchEdge}`, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 6 }}>
        <b style={{ fontFamily: "'Press Start 2P', monospace", fontSize: 10 }}>LITTLE JUNIOR TOWN</b>
        <span style={{ fontSize: 12, color: C.inkSoft }}>⬆⬇⬅➡/WASD 이동 · 문 앞에서 <b>Space</b> 입장 · 강 건너 🌴치앙마이</span>
      </div>

      <div ref={vpRef} style={{ position: "relative", height: 480, overflow: "hidden",
        ...
        <div style={{ position: "absolute", width: WORLD.w, height: WORLD.h, left: -camX, top: -camY }}>
          ...
          {/* 주민센터 기준 방향 표지판 */}
          <div style={{ position: "absolute", left: 1180, top: 860, transform: "translate(-50%,0)" }}><GuidePost /></div>
...
/* ======================= 주민센터(내부 이동) ======================= */
function CenterView({ meetingRooms, chat, onSend, onEnterMeeting, onBack }) {
  ...
  const furniture = [
    ...
    { id: "coffee", x: 40, y: 300, w: 60, h: 66, color: "#8a5a3b", emoji: "☕", label: "커피포트", toast: "따뜻한 커피 한 잔 ☕ 충전 완료!" },
    { id: "vending", x: 300, y: 300, w: 66, h: 70, color: "#d76b96", emoji: "🥤", label: "자판기", toast: "음료를 뽑았습니다 🥤" },
    { id: "water", x: 540, y: 300, w: 56, h: 66, color: "#6fc3e0", emoji: "🚰", label: "정수기", toast: "시원한 물 한 컵 🚰" },
  ];
  return (
    <RoomView title="주민센터" icon="🏛" sub="테이블에서 모여 대화 · 회의실 3곳(예약/잠금)" bg="#f0e4cf" roomW={640} roomH={400} furniture={furniture} onBack={onBack} paused={showChat} headerBg={C.villa}>
      {showChat && (
...
/* ======================= 집(가구 + 메모장) ======================= */
function HomeView({ house, memo, onSaveMemo, onBack }) {
  ...
  return (
    <RoomView title={house.name} icon="🏠" sub="침대·쇼파·티비·책상 · 책상에서 메모 작성" bg="#efe6d2" roomW={640} roomH={400} furniture={furniture} onBack={onBack} paused={open} headerBg={house.wall}>
      {open && (
...
/* ======================= 감사의 방(상점 + 감사 칠판) ======================= */
function ThanksView({ gems, inventory, postits, onBuy, onPost, onBack }) {
  ...
  return (
    <RoomView title="감사의 방" icon="🙏" sub="좌측 선반에서 구매 · 우측 칠판에 감사 포스트잇" bg="#efe6d2" roomW={640} roomH={400} furniture={furniture} onBack={onBack} paused={shop || board} headerBg="#e0a13d">
      {shop && (
...
/* ======================= 마음의 방(마음우체통) ======================= */
function HeartView({ gems, worries, onPost, onBack }) {
  const [open, setOpen] = useState(false);
  ...
  return (
    <RoomView title="마음의 방" icon="💌" sub="퇴근길코어 고민을 익명으로 털어놓는 곳" bg="#efe0e6" roomW={640} roomH={400} furniture={furniture} onBack={onBack} paused={open} headerBg="#d76b96">
      {open && (
...
function ListeningView({ onBack }) {
  ...
  const banner = (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "#241a33", color: "#ffe680", borderBottom: `3px solid ${C.ink}`, flexWrap: "wrap" }}>
      ...
      <span style={{ fontSize: 10, color: "#b9a7d6", marginLeft: "auto" }}>* BGM은 시각 데모</span>
    </div>
  );

  return (
    <RoomView title="리스닝 방" icon="🎵" sub="디제이 부스에서 선곡 · 관객석에서 감상" bg="#2a2140" roomW={640} roomH={400} furniture={furniture} onBack={onBack} paused={open} headerBg="#5b8def" banner={banner}>
      {open && (
...
/* ======================= 릴스방(핸드폰 · 동물/쾌감/밈) ======================= */
function ReelsView({ onBack }) {
  const [open, setOpen] = useState(null);
  const reels = {
    animal: {
      label: "동물", bg: "#bfe3c8", title: "댕댕이·냥이 모음.zip 🐾",
      content: <div style={{ fontSize: 40, lineHeight: 1.3, textAlign: "center" }}>🐶🐱<br />🐰🐼<br />🦊🐹</div>,
    },
    satisfy: {
      label: "쾌감", bg: "repeating-linear-gradient(45deg,#ff8fb1 0 16px,#ffd36b 16px 32px,#8fd0ff 32px 48px,#b6f0c0 48px 64px)", title: "끝없이 보게 되는 쾌감 영상 ✨",
      content: <div style={{ fontSize: 44 }}>🌈✨🫧</div>,
    },
    meme: {
      label: "밈", bg: "#1c1c1c", title: "밈 甲.jpg 😂",
      content: (
  ...
  const furniture = [
    { id: "animal", x: 70, y: 120, w: 90, h: 150, color: "#3fa07a", emoji: "📱", label: "동물", onInteract: () => setOpen("animal") },
    { id: "satisfy", x: 275, y: 120, w: 90, h: 150, color: "#5b8def", emoji: "📱", label: "쾌감", onInteract: () => setOpen("satisfy") },
    { id: "meme", x: 480, y: 120, w: 90, h: 150, color: "#e0a13d", emoji: "📱", label: "밈", onInteract: () => setOpen("meme") },
  ];
  return (
    <RoomView title="릴스방" icon="📱" sub="핸드폰을 눌러 릴스 감상 · 동물 / 쾌감 / 밈" bg="#141c26" roomW={640} roomH={400} furniture={furniture} onBack={onBack} paused={!!open} headerBg="#3fa07a">
      {open && (
...
function PoolView({ onBack }) {
  ...
  return <RoomView title="수영장" icon="🏊" sub="시원한 물놀이 · 레인/다이빙/선베드" bg="#bfe6f2" roomW={640} roomH={400} furniture={furniture} onBack={onBack} headerBg="#4bb4d8" />;
}
function GymView({ onBack, onWork }) {
  const furniture = [
    { id: "tread", x: 55, y: 90, w: 120, h: 90, color: "#5a6b78", emoji: "🏃", label: "러닝머신", toast: "유산소 완료! 심박수 상승 🏃" },
    { id: "weight", x: 250, y: 120, w: 130, h: 100, color: "#c0563a", emoji: "🏋️", label: "웨이트 존(+4⭐)", onInteract: onWork, toast: "💪 운동하고 +4⭐ 획득!" },
    { id: "bench", x: 445, y: 90, w: 130, h: 90, color: "#7a8b99", emoji: "🛋️", label: "벤치프레스", toast: "가슴 운동 3세트 완료 💪" },
    { id: "yoga", x: 250, y: 290, w: 120, h: 60, color: "#7bbf8f", emoji: "🧘", label: "요가매트", toast: "스트레칭으로 마무리 🧘" },
  ];
  return <RoomView title="헬스장" icon="💪" sub="러닝머신·웨이트·요가 · 웨이트 존에서 젬 획득" bg="#e6e2da" roomW={640} roomH={400} furniture={furniture} onBack={onBack} headerBg="#c0563a" />;
}

/* ======================= 흡연의 방(플레이버) ======================= */
function SmokeView({ onBack }) {
  const furniture = [
  ...
  return <RoomView title="흡연의 방" icon="🚬" sub="흡연 중 💨 · 환기 필수" bg="#dfe3e6" roomW={640} roomH={400} furniture={furniture} onBack={onBack} headerBg="#7a8b99" />;
}
  ...
  return (
    <Panel style={{ padding: 0, overflow: "hidden" }}>
      ...
      <div style={{ padding: 20, textAlign: "center", background: `repeating-linear-gradient(0deg, ${C.parch} 0 40px, ${C.parchLine} 40px 80px)` }}>
        ...
        {rented ? (
          <div style={{ background: C.good, color: C.white, border: `3px solid ${C.ink}`, padding: 12, fontSize: 14 }}>✅ 이미 렌트한 숙소입니다. 편히 쉬세요!</div>
        ) : (
  ...
  return (
    <div style={{ fontFamily: "'DotGothic16', monospace", minHeight: "100vh", background: `repeating-linear-gradient(45deg, ${C.grass} 0 24px, ${C.grassDark} 24px 48px)`, color: C.ink, padding: 14, boxSizing: "border-box" }}>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto 12px" }}>
        <Panel style={{ padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontFamily: "'Press Start 2P', monospace", fontSize: 11, color: C.inkSoft }}>OPEN WORLD</div>
              <div style={{ fontSize: 13, color: C.inkSoft }}>작은 주니어 네이버 · 오픈월드 에디션</div>
            </div>
          </div>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        {view === "world" && <WorldView pos={worldPos} setPos={setWorldPos} day={day} gems={gems} rentedHouses={rented} onEnter={handleEnter} onNextDay={nextDay} />}
        {view === "center" && <CenterView meetingRooms={meetingRooms} chat={centerChat} onSend={(t) => setCenterChat((c) => [...c, { who: "나", text: t, me: true }])} onEnterMeeting={(id) => { setMeetingId(id); setView("meeting"); }} onBack={backToWorld} />}
        ...
        {view === "house" && houseMeta && <HomeView house={houseMeta} memo={memos[houseId]} onSaveMemo={(t) => setMemos((m) => ({ ...m, [houseId]: t }))} onBack={backToWorld} />}
        {view === "thanks" && <ThanksView gems={gems} inventory={thanksInv} postits={postits} onBuy={(it) => { setGems((g) => g - it.price); setThanksInv((v) => [...v, it]); }} onPost={(p) => setPostits((v) => [...v, { ...p, id: Date.now() }])} onBack={backToWorld} />}
        {view === "heart" && <HeartView gems={gems} worries={worries} onPost={(text, cost) => { setGems((g) => g - cost); setWorries((w) => [{ id: Date.now(), text }, ...w]); }} onBack={backToWorld} />}
        {view === "listening" && <ListeningView onBack={backToWorld} />}
        {view === "reels" && <ReelsView onBack={backToWorld} />}
        {view === "pool" && <PoolView onBack={backToWorld} />}
        {view === "gym" && <GymView onBack={backToWorld} onWork={() => award(4)} />}
        {view === "smoke" && <SmokeView onBack={backToWorld} />}
