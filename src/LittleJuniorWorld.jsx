...
list.push({ id: "jjeop", kind: "small", x: 1820, y: 1210, r: 55, label: "🍴 쩝쩝박사", tint: "#c0563a" });
  ...
  list.push({ id: "airportIC", kind: "airport", side: "town", x: 2020, y: 745, r: 70, label: "✈️ 인천공항" });
  list.push({ id: "airportCM", kind: "airport", side: "cm", x: 2300, y: 745, r: 70, label: "✈️ 치앙마이공항" });
...
function Airport({ size = 150, tint = "#5b8def", tintDk = "#3a5fa8", label = "인천" }) {
  return (
...
function GuardGate({ onPass, onClose, passed = false, side = "town" }) {
  ...
  const submit = () => {
    if (code.trim().toLowerCase() === "chiang") { setOk(true); onPass(); setTimeout(onClose, 1100); }
          ...
          {passed ? (
            <>
              ...
              <div style={{ fontSize: 12.5, color: C.inkSoft, textAlign: "center", lineHeight: 1.8, marginBottom: 12 }}>
                입국 심사를 이미 통과했어요.<br />이제 다리로 자유롭게 왕복할 수 있습니다.
              </div>
              <div style={{ background: C.white, border: `2px dashed ${C.ink}`, borderRadius: 8, padding: 11, fontSize: 12.5, lineHeight: 1.8, marginBottom: 12 }}>
                🛫 {from} → 🛬 {to}<br />
                <span style={{ color: C.inkSoft }}>다리 위로 걸어가면 바로 건널 수 있어요</span>
              </div>
              <PxButton tone="good" onClick={onClose} style={{ width: "100%", padding: 11, fontSize: 14 }}>확인</PxButton>
            </>
          ) : ok ? (
            <div style={{ textAlign: "center", fontSize: 15, margin: "12px 0", color: C.good, fontWeight: "bold" }}>
              ✅ 입국 심사 통과!<br />
              <span style={{ fontSize: 12, color: C.inkSoft, fontWeight: "normal" }}>이제부터 자유롭게 왕복할 수 있어요</span>
            </div>
          ) : (
            <>
              <div style={{ fontSize: 14, textAlign: "center", margin: "8px 0 4px", fontWeight: "bold" }}>입국 심사</div>
              <div style={{ fontSize: 11.5, color: C.inkSoft, textAlign: "center", marginBottom: 10, lineHeight: 1.7 }}>
                여권 번호를 입력하면 <b>한 번만</b> 심사받으면 돼요.<br />통과 후에는 계속 왔다갔다 할 수 있습니다.
              </div>
              <input value={code} onChange={(e) => { setCode(e.target.value); setErr(false); }} onKeyDown={(e) => { if (e.key === "Enter") submit(); }} autoFocus
                placeholder="여권 번호" style={{ width: "100%", boxSizing: "border-box", padding: 10, border: `3px solid ${C.ink}`, fontFamily: "'DotGothic16', monospace", fontSize: 15, background: C.white, textAlign: "center" }} />
              {err && <div style={{ color: C.danger, fontSize: 12, marginTop: 6, textAlign: "center" }}>번호가 달라요. 힌트: chiang</div>}
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                <PxButton tone="ink" onClick={onClose} style={{ flex: 1, padding: 9, fontSize: 13 }}>돌아가기</PxButton>
                <PxButton tone="good" onClick={submit} style={{ flex: 1, padding: 9, fontSize: 13 }}>제출</PxButton>
              </div>
            </>
          )}
        </Panel>
...
function WorldView({ pos, setPos, day, gems, sprites = {}, cutCfg = {}, look = null, carry = null, shuffle = false, onShuffle, onNextTrack, onPrevTrack, rentedHouses, onEnter, onNextDay, bgm, onToggleBgm, onRequestSong, bubble, townRain = false, cmRain = false, tracks = [], onSelectTrack, outfit = null, vehicle = null, houseSkin = null, isMyHouse = () => false, others = {}, netCount = 1, netStatus = "", facingRef = null, bgmVol = 0.6, onBgmVol = null, danceRef = null, onGift = null, myNick = "" }) {
  ...
  const spriteFor = (o) => {
    ...
    if (o.kind === "airport") return <Airport size={150} tint={o.side === "cm" ? "#3fa07a" : "#5b8def"} tintDk={o.side === "cm" ? "#1d6b4a" : "#3a5fa8"} label={o.side === "cm" ? "치앙마이" : "인천"} />;
...
const HELP_DATA = {
  ...
  "👥 소통": [
    ...
    { icon: "✈️", title: "치앙마이 가는 법", body: "다리 양 끝에 ✈️ 인천공항 · ✈️ 치앙마이공항이 있어요. 공항에서 여권 번호로 입국 심사를 한 번만 통과하면, 그 뒤로는 다리로 자유롭게 왕복할 수 있습니다. 여권 번호가 궁금하면 📋 게시판 공지를 확인하거나 마을 주민에게 물어보세요. 확성기로 외쳐보는 것도 방법!", go: "board", goLabel: "게시판 가기" },
  ],
  "💎 젬·🪙 골드": [
...
/* ☰ 메뉴 (마을주민들 + 피드백) */
function MenuSheet({ onClose, people, onDm, onCall, sprites, userSprites, cutCfg, onSetCut, onSetSprite, onClearSprite, onClearSprites }) {
...
export default function App() {
  ...
  /* 🧑 내 프로필 (직업 · 아이콘 · 외모) */
  const [profile, setProfile] = useState(() => loadJSON("echotown_profile", { job: "", avatar: "🧑‍💻", look: DEFAULT_LOOK }));
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      dbSaveProfile(myName, { gems, gold, exp, lifetime, profile, homeGifts, fridge, outfit, owned, ikeaOwned, houseSkin, vehicle, myFurni, thanksInv, memos, stats, housePw, couponDone, qNotes, qAccept });
    }, 2000);
