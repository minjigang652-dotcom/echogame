...
/* ===== 🌊 바다 (별도 맵) ===== */
function SeaView({ pos, setPos, onBack, onFish, look = null, carry = null, pet = null, outfit = null, bubble = null, others = {} }) {
  const [facing, setFacing] = useState(1);
  const [moving, setMoving] = useState(false);
  const [near, setNear] = useState(null);
  const keys = useRef({});
  ...
  const SPOTS = [
    ...
    { id: "light", x: SEA_PIER_X - 110, y: SEA_PIER_END - 60, r: 70, emoji: "🗼", label: "등대" },
  ...
  useEffect(() => {
    const down = (e) => {
      ...
      const raw = e.key.toLowerCase();
      if (["arrowup", "arrowdown", "arrowleft", "arrowright", " ", "w", "a", "s", "d"].includes(raw)) e.preventDefault();
      if (raw === " ") { const n = nearRef.current; if (n) act(n); return; }
    ...
    <Panel style={{ padding: 0, overflow: "hidden" }}>
      ...
      <div ref={vpRef} tabIndex={0} className="game-vp" style={{ position: "relative", height: 480, overflow: "hidden", outline: "none", background: "#3a8fc4" }}>
        <div style={{ position: "absolute", width: SEA.w, height: SEA.h, left: -camX, top: -camY }}>
          ...
          {/* 해변 소품 */}
          {[[180, 190, "⛱️", 40], [380, 210, "🏖️", 34], [1080, 195, "⛱️", 38], [1320, 215, "🏐", 26], [560, 225, "🐚", 22], [900, 220, "🩴", 24], [1450, 200, "🦀", 26]].map(([x, y, e, sz], i) => (
...
const HELP_DATA = {
  ...
  "🎮 놀이": [
    ...
    { icon: "🎣", title: "낚시터", body: "바다 선착장 끝에 있어요. 낚싯대를 던지고 ❗ 입질이 오면 1.4초 안에 당기세요. 🐟 🐠 🦑 🐙 … 잡은 만큼 🪙 골드를 받아요." },
    ...
    <div style={{ fontFamily: "'DotGothic16', monospace", minHeight: "100vh", background: `repeating-linear-gradient(45deg, ${C.grass} 0 24px, ${C.grassDark} 24px 48px)`, color: C.ink, padding: 14, boxSizing: "border-box" }}>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        {view === "sea" && <SeaView pos={seaPos} setPos={setSeaPos} others={netOthers} bubble={bubble}
          ...
          onFish={() => setView("fishing")}
          onBack={() => { setWorldPos({ x: 1312, y: 1400 }); backToWorld(); }} />}
        {view === "fishing" && <FishingView onBack={() => setView("sea")} bubble={bubble} best={stats.fish || 0}
