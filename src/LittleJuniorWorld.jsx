...
const APP_VERSION = "v46 · 2026-07-24";
...
list.push({ id: "jjeop", kind: "small", x: 1820, y: 1210, r: 55, label: "🍴 쩝쩝박사", tint: "#c0563a" });
  ...
  list.push({ id: "fishing", kind: "small", x: 1312, y: 1930, r: 66, label: "🎣 낚시터" });
  // 은행 / 게시판
...
const WORLD = { w: 2620, h: 2120 };
const RIVER_X = 2140, RIVER_W = 120;
/* 🌊 남쪽 바다 : 모래사장(SAND_Y~SEA_Y) 아래는 물이라 못 걸어요.
   단, 십자로 세로길 끝에 놓인 선착장(PIER) 위로는 걸어갈 수 있어요. */
const SAND_Y = 1560, SEA_Y = 1700;
const PIER_X = 1266, PIER_W = 92, PIER_END = 2010;
...
/* 🎣 낚시터 — 선착장 끝에서 바다 낚시 */
const FISH_CATCH = [
  { emoji: "🐟", name: "작은 물고기", gold: 2, w: 34 },
  { emoji: "🐠", name: "열대어", gold: 4, w: 22 },
  { emoji: "🦐", name: "새우", gold: 2, w: 14 },
  { emoji: "🦑", name: "오징어", gold: 6, w: 10 },
  { emoji: "🐡", name: "복어", gold: 8, w: 8 },
  { emoji: "🦀", name: "꽃게", gold: 7, w: 7 },
  { emoji: "🐙", name: "문어", gold: 12, w: 3 },
  { emoji: "🥾", name: "낡은 장화", gold: 0, w: 2 },
];
function pickCatch() {
  const total = FISH_CATCH.reduce((a, f) => a + f.w, 0);
  let r = Math.random() * total;
  for (const f of FISH_CATCH) { r -= f.w; if (r <= 0) return f; }
  return FISH_CATCH[0];
}

function FishingView({ onBack, bubble, onReward, best = 0, onRecord }) {
  ...
  const cast = () => {
    ...
    timer.current = setTimeout(() => {
      setPhase("bite");
      clearTimeout(timer.current);
      timer.current = setTimeout(() => setPhase("miss"), 1400);   // 1.4초 안에 당겨야 해요
  ...
  const pull = () => {
    ...
    setLog((l) => [{ ...f, id: Date.now() }, ...l].slice(0, 8));
    if (f.gold > 0) onReward && onReward(f.gold);
    onRecord && onRecord();
  };

  const furniture = [
    { id: "spot", x: 270, y: 150, w: 110, h: 110, color: "transparent", emoji: "🎣", label: "낚시 자리", onInteract: () => { if (phase === "idle" || phase === "result" || phase === "miss") cast(); else pull(); } },
    { id: "bucket", x: 90, y: 250, w: 70, h: 70, color: "transparent", emoji: "🪣", label: `양동이 (${log.length})`, toast: log.length ? `🪣 오늘 잡은 것 : ${log.map((x) => x.emoji).join(" ")}` : "🪣 아직 비어 있어요" },
    { id: "tower", x: 470, y: 120, w: 90, h: 120, color: "transparent", emoji: "🗼", label: "등대", toast: "🗼 멀리 배들이 지나가요" },
  ];

  return (
    <RoomView title="낚시터" icon="🎣" sub="선착장 끝 · 입질이 오면 바로 당기세요" bg="#4fa8d8" roomW={640} roomH={400}
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", padding: 14, zIndex: 20, background: "rgba(0,0,0,0.25)" }}>
        <div style={{ width: "100%", maxWidth: 380 }}>
          <Panel style={{ padding: 16, textAlign: "center" }}>
            <div style={{ fontSize: 12, color: C.inkSoft }}>🎣 낚시터 · 최고 기록 {best}마리</div>

            <div style={{ height: 120, display: "flex", alignItems: "center", justifyContent: "center", margin: "10px 0" }}>
              ...
              {phase === "result" && got && (
                <div>
                  ...
                  <div style={{ fontSize: 13, color: got.gold ? C.good : C.inkSoft, marginTop: 3 }}>{got.gold ? `🪙 ${got.gold} 획득!` : "…이건 좀 아쉽네요"}</div>
                </div>
              )}
              {phase === "miss" && <div><div style={{ fontSize: 50 }}>💨</div><div style={{ fontSize: 13.5, color: C.inkSoft, marginTop: 6 }}>놓쳤어요! 다시 던져볼까요</div></div>}
            </div>

            <PxButton tone={phase === "bite" ? "danger" : "good"} onClick={() => (phase === "wait" || phase === "bite" ? pull() : cast())}
              style={{ width: "100%", padding: 14, fontSize: 15 }}>
              {phase === "bite" ? "🎣 당기기!" : phase === "wait" ? "기다리는 중… (누르면 헛챔질)" : "🎣 낚싯대 던지기"}
            </PxButton>

            {log.length > 0 && (
              <div style={{ marginTop: 12 }}>
                <div style={{ fontSize: 11, color: C.inkSoft, marginBottom: 5 }}>🪣 오늘 잡은 것 {log.length}마리</div>
                <div style={{ display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "center" }}>
                  {log.map((x) => <span key={x.id} title={x.name} style={{ fontSize: 20 }}>{x.emoji}</span>)}
                </div>
              </div>
            )}
            <PxButton tone="ink" onClick={onBack} style={{ width: "100%", marginTop: 12, padding: 11, fontSize: 13 }}>나가기</PxButton>
...
const MAP_ZONES = [
  ...
  { label: "바다", color: "#3a7bb5", x1: 60, y1: 1620, x2: 2560, y2: 2080 },
];
...
function WorldView({ pos, setPos, day, gems, sprites = {}, cutCfg = {}, look = null, carry = null, pet = null, shuffle = false, onShuffle, onNextTrack, onPrevTrack, onReconnect, onDismount, rentedHouses, onEnter, onNextDay, bgm, onToggleBgm, onRequestSong, bubble, townRain = false, cmRain = false, tracks = [], onSelectTrack, outfit = null, vehicle = null, houseSkin = null, isMyHouse = () => false, others = {}, netCount = 1, netStatus = "", facingRef = null, bgmVol = 0.6, onBgmVol = null, danceRef = null, onGift = null, myNick = "" }) {
  ...
  useEffect(() => {
    ...
    const loop = (now) => {
      ...
      if (dx || dy) {
        ...
        if (!canCross && y < SAND_Y) {
          const L = RIVER_X - 6, R = RIVER_X + RIVER_W + 6;
          if (x > L && x < R) x = px <= L ? L : R;
        }
        // 🌊 바다는 헤엄칠 수 없어요. 선착장 위로만 걸어갈 수 있습니다.
        const onPier = x > PIER_X + 6 && x < PIER_X + PIER_W - 6;
        if (y > SEA_Y - 12) {
          if (onPier) y = Math.min(y, PIER_END - 20);
          else y = SEA_Y - 12;
        }
        posRef.current = { x, y }; setPos({ x, y });
        if (dx < 0) setFacing(-1); else if (dx > 0) setFacing(1);
  ...
  const spriteFor = (o) => {
    ...
    if (o.id === "fishing") return <span style={{ fontSize: 64, display: "block", filter: "drop-shadow(0 3px 2px rgba(0,0,0,0.35))" }}>🎣</span>;
    ...
    <Panel style={{ padding: 0, overflow: "hidden" }}>
      ...
      <div ref={vpRef} tabIndex={0} onMouseDown={focusGame} className="game-vp" style={{ position: "relative", height: 480, overflow: "hidden", outline: "none",
        ...
        <div style={{ position: "absolute", width: WORLD.w, height: WORLD.h, left: -camX, top: -camY }}>
          {/* 🌊 남쪽 바다 : 모래사장 → 얕은 물 → 깊은 바다 */}
          <div style={{ position: "absolute", left: 0, top: SAND_Y, width: WORLD.w, height: SEA_Y - SAND_Y,
            background: "linear-gradient(180deg,#efe0b8 0%,#e6d3a3 55%,#dcc68f 100%)", borderTop: "5px solid #cdb47e" }} />
          <div style={{ position: "absolute", left: 0, top: SEA_Y, width: WORLD.w, height: WORLD.h - SEA_Y,
            background: "linear-gradient(180deg,#6fc3e0 0%,#3a8fc4 45%,#1f5f96 100%)" }} />
          {/* 밀려오는 파도 */}
          <div className="sea-wave" style={{ position: "absolute", left: 0, top: SEA_Y - 10, width: WORLD.w, height: 22,
            background: "repeating-linear-gradient(90deg, rgba(255,255,255,0.75) 0 26px, transparent 26px 62px)", opacity: 0.8 }} />
          <div className="sea-wave2" style={{ position: "absolute", left: 0, top: SEA_Y + 44, width: WORLD.w, height: 14,
            background: "repeating-linear-gradient(90deg, rgba(255,255,255,0.45) 0 34px, transparent 34px 78px)" }} />
          <div className="sea-wave" style={{ position: "absolute", left: 0, top: SEA_Y + 130, width: WORLD.w, height: 12,
            background: "repeating-linear-gradient(90deg, rgba(255,255,255,0.3) 0 46px, transparent 46px 96px)" }} />
          {/* 해변 소품 */}
          {[[420, 1600, "⛱️", 42], [700, 1618, "🏖️", 36], [980, 1600, "🩴", 26], [1600, 1608, "⛱️", 40], [1900, 1620, "🐚", 24], [2200, 1600, "🏐", 26], [260, 1622, "🐚", 22], [2420, 1614, "🦀", 26]].map(([x, y, e, sz], i) => (
            <span key={"bch" + i} style={{ position: "absolute", left: x, top: y, fontSize: sz }}>{e}</span>
          ))}
          {/* 바다 위 */}
          {[[520, 1790, "⛵", 44], [1850, 1830, "🚤", 40], [2380, 1770, "🛥️", 38], [820, 1960, "🐬", 34], [1620, 1980, "🐋", 44], [300, 1900, "🐟", 24], [2100, 1940, "🐠", 26]].map(([x, y, e, sz], i) => (
            <span key={"sea" + i} className="sea-float" style={{ position: "absolute", left: x, top: y, fontSize: sz, animationDelay: `${-i * 1.3}s` }}>{e}</span>
          ))}
          {/* 선착장 (십자로 세로길이 바다까지 이어져요) */}
          <div style={{ position: "absolute", left: PIER_X, top: SEA_Y - 30, width: PIER_W, height: PIER_END - SEA_Y + 30,
            background: "repeating-linear-gradient(0deg,#a9814a 0 12px,#8a6b3f 12px 24px)", borderLeft: "4px solid #7a5230", borderRight: "4px solid #7a5230", boxShadow: "0 6px 14px rgba(0,0,0,0.3)" }} />
          <span style={{ position: "absolute", left: PIER_X - 34, top: PIER_END - 70, fontSize: 40 }}>🗼</span>

          {/* 흙길: 중앙 광장 십자 */}
          <div style={{ position: "absolute", left: 1290, top: 0, width: 44, height: SAND_Y, background: `repeating-linear-gradient(0deg, ${C.path} 0 10px, ${C.pathDark} 10px 20px)` }} />
          ...
          {/* 강 + 다리(다리에서만 건널 수 있음) */}
          <div style={{ position: "absolute", left: RIVER_X, top: 0, width: RIVER_W, height: SAND_Y,
...
const HELP_DATA = {
  ...
  "🎮 놀이": [
    { icon: "🎣", title: "낚시터", body: "십자로 세로길을 따라 남쪽 끝까지 내려가면 🌊 바다와 선착장이 나와요. 선착장 끝 낚시터에서 낚싯대를 던지고 입질이 오면 바로 당기세요. 잡은 만큼 🪙 골드를 받아요.", go: "fishing", goLabel: "낚시터 가기" },
...
function EchoTown() {
  ...
  payloadRef.current = { gems, gold, exp, lifetime, townRegion, profile, skills, schoolDone, pets, activePet, fishes, facilities, homeLayout, homeGifts, fridge, outfit, owned, ikeaOwned, houseSkin, vehicle, myFurni, thanksInv, memos, stats, housePw, couponDone, qNotes, qAccept };
  ...
  }, [myName, gems, gold, exp, lifetime, townRegion, profile, skills, schoolDone, pets, activePet, fishes, facilities, homeLayout, homeGifts, fridge, outfit, owned, ikeaOwned, houseSkin, vehicle, myFurni, thanksInv, memos, stats, housePw, couponDone, qNotes, qAccept, flushSave]);
  ...
  return (
    ...
    <div style={{ fontFamily: "'DotGothic16', monospace", minHeight: "100vh", background: `repeating-linear-gradient(45deg, ${C.grass} 0 24px, ${C.grassDark} 24px 48px)`, color: C.ink, padding: 14, boxSizing: "border-box" }}>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        {view === "fishing" && <FishingView onBack={backToWorld} bubble={bubble} best={stats.fish || 0}
          onReward={(n) => awardGold(n)} onRecord={() => bump("fish")} />}
