...
import NaverSchoolPanel from "./NaverSchool.jsx";
import { School, SchoolView } from "./VideoSchool.jsx";
...
export function PixelHouse({ roof, roofDk, wall, size = 96 }) {
  return (
...
export function PxButton({ children, onClick, disabled, tone = "wood", style, title }) {
  const tones = {
...
export function TitleBar({ icon, title, sub, onBack, right, bg = C.parch, fg = C.ink, tipId = null }) {
  return (
...
function RoomView({ title, icon, sub, bg, roomW = 640, roomH = 400, furniture, start, onBack, paused = false, children, headerBg = C.parch, banner = null, bubble = null, outfit = null, look = null, carry = null, pet = null, tipId = null, editable = false, onMoveFurni, side = null, sitAt = null, onStand = null }) {
  ...
  const roomElRef = useRef(null);
  const startDrag = (e, f) => {
    const box = roomElRef.current && roomElRef.current.getBoundingClientRect();
    if (!box) return;
    dragRef.current = { id: f.id, dx: e.clientX - box.left - f.x, dy: e.clientY - box.top - f.y, w: f.w, h: f.h };
    try { e.currentTarget.setPointerCapture(e.pointerId); } catch (err) {}
  };
  const onDragMove = (e) => {
    const d = dragRef.current; if (!d) return;
    const box = roomElRef.current && roomElRef.current.getBoundingClientRect(); if (!box) return;
    const nx = Math.round(Math.max(0, Math.min(roomW - d.w, e.clientX - box.left - d.dx)) / 5) * 5;
    const ny = Math.round(Math.max(0, Math.min(roomH - d.h, e.clientY - box.top - d.dy)) / 5) * 5;
    onMoveFurni && onMoveFurni(d.id, nx, ny);
  };
...
const PETSHOP_TABS = { fac: "🏗 시설", pet: "🐾 동물", fish: "🐠 물고기" };
...
function PetShop({ onBack, gold, pets = [], activePet = null, fishes = [], facilities = [], onBuyPet, onSetActive, onBuyFish, onBuyFacility, onCare, bubble }) {
  /* 무신사·이케아처럼 : 처음엔 가게 안을 돌아다니고, 코너 앞에서 눌러야 목록이 열려요 */
  const [tab, setTab] = useState(null);
  const [pick, setPick] = useState(null);   // 눌러서 자세히 보기
  const has = (id) => pets.includes(id);
  const hasAqua = facilities.includes("aquarium");
  const hasYard = facilities.includes("yard");
  const furniture = [
    { id: "hyunguk", x: 285, y: 55, w: 50, h: 82, npc: true, facing: 1, label: "사장 형욱", outfit: { top: { color: "#7bbf8f" }, bottom: { color: "#4a382a" }, shoes: { color: "#2a1e14" } }, toast: "어서와요! 마당·수족관부터 보고 가세요 🐾" },
    { id: "fac", x: 70, y: 130, w: 130, h: 110, color: "#e0a13d", emoji: "🏗", label: "시설 코너", onInteract: () => setTab("fac") },
    { id: "pet", x: 260, y: 190, w: 130, h: 110, color: "#7bbf8f", emoji: "🐕", label: "입양 코너", onInteract: () => setTab("pet") },
    { id: "fish", x: 450, y: 130, w: 130, h: 110, color: "#5b8def", emoji: "🐠", label: "수조 코너", onInteract: () => setTab("fish") },
  ];

  const detailFor = (kind, x) => setPick({ kind, x });

  return (
    <RoomView tipId="petshop" title="형욱이네" icon="🐾" sub="코너 앞에서 눌러 입양하고 🐠 물고기를 데려가요" bg="#f3ead8" roomW={640} roomH={400}
      furniture={furniture} onBack={onBack} paused={!!tab} headerBg="#7bbf8f" bubble={bubble}
      banner={(
        <div style={{ padding: "8px 14px", background: "#eaf5ee", borderBottom: `3px solid ${C.ink}`, fontSize: 11.5, color: C.inkSoft, lineHeight: 1.6 }}>
          🏗 시설 · 🐾 입양 · 🐠 수조 <b style={{ color: C.ink }}>세 코너</b>로 걸어가서 눌러보세요 · 미보유는 ❓ 로 보이고 누르면 설명이 나와요
        </div>
      )}>

      {tab && (
        <RoomModal title={`🐾 형욱이네 · ${PETSHOP_TABS[tab]}`} onClose={() => { setTab(null); setPick(null); }} maxW={460}>
          <div style={{ display: "flex", gap: 5, marginBottom: 10 }}>
            {Object.keys(PETSHOP_TABS).map((k) => (
              <PxButton key={k} tone={tab === k ? "good" : "wood"} onClick={() => { setTab(k); setPick(null); }} style={{ flex: 1, fontSize: 11.5, padding: 9 }}>{PETSHOP_TABS[k]}</PxButton>
            ))}
          </div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 9 }}>
            <span style={{ fontSize: 11, color: C.inkSoft }}>❓ 를 누르면 이 방에서 할 수 있는 게 나와요</span>
          ...
          {tab === "fac" && (
            <>
              <div style={{ fontSize: 11.5, color: C.inkSoft, marginBottom: 8, lineHeight: 1.7 }}>
                반려동물·물고기를 데려오려면 <b>먼저 살 곳</b>을 마련해야 해요. 눌러서 자세히 볼 수 있어요.
              </div>
                ...
                {FACILITIES.map((fc) => {
                  ...
                  return (
                    <div key={fc.id} style={{ display: "flex", alignItems: "center", gap: 10, background: own ? "#eef6ef" : C.white, border: `3px solid ${own ? C.good : C.ink}`, borderRadius: 10, padding: 11 }}>
                      <button type="button" onClick={() => detailFor("fac", fc)} title="자세히 보기"
                        style={{ cursor: "pointer", background: "none", border: "none", fontSize: 34, padding: 0, lineHeight: 1 }}>{fc.emoji}</button>
                      <div style={{ flex: 1, minWidth: 0, cursor: "pointer" }} onClick={() => detailFor("fac", fc)}>
            ...
            <>
              ...
              {!hasYard && (
                <div style={{ background: "#fbe4e0", border: `3px solid ${C.danger}`, borderRadius: 10, padding: 12, marginBottom: 10, textAlign: "center" }}>
                  ...
                  <PxButton tone="gold" onClick={() => setTab("fac")} style={{ width: "100%", marginTop: 9, padding: 10, fontSize: 12.5 }}>🏗 시설 코너에서 마당 구입하기</PxButton>
                </div>
              )}
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(126px, 1fr))", gap: 8, maxHeight: 300, overflow: "auto" }}>
                {PETS.map((pt) => {
            ...
            <>
              ...
              {!hasAqua && (
                <div style={{ background: "#e0f0fb", border: `3px solid #3a7bb5`, borderRadius: 10, padding: 12, marginBottom: 10, textAlign: "center" }}>
                  ...
                  <PxButton tone="blue" onClick={() => setTab("fac")} style={{ width: "100%", marginTop: 9, padding: 10, fontSize: 12.5 }}>🏗 시설 코너에서 수족관 구입하기</PxButton>
                </div>
              )}
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))", gap: 8, maxHeight: 300, overflow: "auto" }}>
                {FISHES.map((f) => {
                  ...
                  return (
                    <div key={f.id} style={{ background: own ? C.white : "#efe9dc", border: `3px solid ${own ? C.ink : "#b5ad9c"}`, borderRadius: 10, padding: 10, textAlign: "center" }}>
                      ...
                      {own && <div style={{ fontSize: 10, color: C.good, fontWeight: "bold", marginTop: 2 }}>어항에 {cnt}마리</div>}
          ...
          {pick && (() => {
            const x = pick.x;
            const own = pick.kind === "fac" ? facilities.includes(x.id) : pick.kind === "pet" ? has(x.id) : fishes.filter((y) => y === x.id).length > 0;
            const how = pick.kind === "fac" ? "형욱이네 🏗 시설 코너에서 🪙 골드로 구입" : pick.kind === "pet" ? "🌳 마당을 먼저 사고, 🐾 입양 코너에서 입양" : "🐟 수족관을 먼저 사고, 🐠 수조 코너에서 데려오기";
            const detail = pick.kind === "fish"
              ? "우리 집 수족관에서 헤엄쳐요. 집에 들어가 수족관을 누르면 밥도 줄 수 있어요."
              : (x.desc || "");
            return (
              <div onClick={() => setPick(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.62)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 150, padding: 14 }}>
                <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 320 }}>
                  <div style={{ background: C.parch, border: `4px solid ${C.ink}`, borderRadius: 14, padding: 20, textAlign: "center" }}>
                    <div style={{ fontSize: 54, filter: own ? "none" : "grayscale(1)", opacity: own ? 1 : 0.5 }}>{own ? x.emoji : "❓"}</div>
                    <div style={{ fontSize: 17, fontWeight: "bold", margin: "8px 0 6px" }}>{own ? x.name : "???"}</div>
                    <div style={{ fontSize: 12.5, color: C.inkSoft, lineHeight: 1.8 }}>
                      {own ? detail : "아직 데려오지 않았어요. 아래 조건을 채우면 열려요."}
                    </div>
                    <div style={{ background: C.white, border: `2px dashed ${C.ink}`, borderRadius: 8, padding: 10, fontSize: 12, lineHeight: 1.7, margin: "12px 0" }}>
                      📌 얻는 방법<br /><b>{how}</b><br />
                      <span style={{ color: "#a86e13", fontWeight: "bold" }}>🪙 {x.price} 골드</span>
                    </div>
                    <div style={{ fontSize: 13, fontWeight: "bold", color: own ? C.good : C.danger, marginBottom: 12 }}>
                      {own ? "✔ 보유중이에요" : `🔒 아직 없어요 · 보유 🪙${fmt(gold)}`}
                    </div>
...
const RODS = [
  ...
  { id: 1, name: "튼튼한 낚싯대", emoji: "🎣", price: 90,  desc: "희귀한 게 조금 더 잘 걸려요" },
  { id: 2, name: "전설의 낚싯대", emoji: "🔱", price: 260, desc: "대물과 보물이 훨씬 잘 걸려요" },
];
...
function pickCatch(rod) {
  const list = catchWeights(rod);
  const total = list.reduce((a, f) => a + f.ww, 0);
  let r = Math.random() * total;
  for (const f of list) { r -= f.ww; if (r <= 0) return f; }
  return list[0];
}
const BOX_GIFTS = [
...
/* 📖 바다 도감 */
function SeaDex({ rod = 0, caught = {}, onClose }) {
  const list = catchWeights(rod);
  const total = list.reduce((a, f) => a + f.ww, 0);
  const TYPE = { fish: ["🐟 물고기", "#3fa07a"], trash: ["🗑 쓰레기", "#7a8b99"], gem: ["💎 보물", "#5b8def"], box: ["🎁 특별", "#d76b96"] };
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 140, padding: 14 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 420, maxHeight: "calc(100vh - 28px)", overflowY: "auto" }}>
        <Panel style={{ padding: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
            <span style={{ fontSize: 22 }}>📖</span>
            <b style={{ flex: 1, fontSize: 15 }}>바다 도감</b>
            <PxButton tone="ink" onClick={onClose} style={{ fontSize: 11, padding: "5px 9px" }}>✕</PxButton>
          </div>
          <div style={{ fontSize: 11, color: C.inkSoft, marginBottom: 9 }}>
            {RODS[rod].emoji} {RODS[rod].name} 기준 확률 · 모은 종류 {list.filter((f) => caught[f.id]).length} / {list.length}
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            {list.map((f) => {
              const have = !!caught[f.id];
              const pct = ((f.ww / total) * 100).toFixed(1);
              const [tn, tc] = TYPE[f.type];
              return (
                <div key={f.id} style={{ display: "flex", alignItems: "center", gap: 9, background: have ? C.white : "#e7e2d6", border: `2px solid ${have ? C.ink : "#b5ad9c"}`, borderRadius: 8, padding: "8px 10px" }}>
                  <span style={{ fontSize: 26, filter: have ? "none" : "grayscale(1)", opacity: have ? 1 : 0.4 }}>{have ? f.emoji : "❔"}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: "bold", color: have ? C.ink : C.inkSoft }}>{have ? f.name : "???"}</div>
                    <span style={{ fontSize: 9.5, background: tc, color: C.white, borderRadius: 8, padding: "1px 7px" }}>{tn}</span>
                    {have && <span style={{ fontSize: 10, color: C.inkSoft, marginLeft: 6 }}>{caught[f.id]}마리 잡음</span>}
                  </div>
                  <div style={{ textAlign: "right", whiteSpace: "nowrap" }}>
                    <div style={{ fontSize: 12, fontWeight: "bold", color: "#a86e13" }}>{f.type === "gem" ? (f.id === "diamond" ? "💎 5" : "💎 1") : `🪙 ${f.sell}`}</div>
          ...
          <div style={{ fontSize: 10.5, color: C.inkSoft, textAlign: "center", marginTop: 10, lineHeight: 1.6 }}>
            💎 보물은 잡는 즉시 젬으로 들어와요 · 나머지는 어부 아저씨에게 팔 수 있어요
          </div>
        </Panel>
      </div>
    </div>
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 420, maxHeight: "calc(100vh - 28px)", overflowY: "auto" }}>
        <Panel style={{ padding: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
            ...
            <b style={{ flex: 1, fontSize: 15 }}>어부 아저씨</b>
            <GemBadge kind="gold" amount={gold} />
            <PxButton tone="ink" onClick={onClose} style={{ fontSize: 11, padding: "5px 9px" }}>✕</PxButton>
          </div>
          <div style={{ fontSize: 11.5, color: C.inkSoft, marginBottom: 10 }}>"미끼 없으면 낚시 못 혀~ 잡은 건 내가 다 사줄게."</div>

          <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
            <PxButton tone={tab === "buy" ? "good" : "wood"} onClick={() => setTab("buy")} style={{ flex: 1, fontSize: 12, padding: 9 }}>🛒 사기</PxButton>
            <PxButton tone={tab === "sell" ? "good" : "wood"} onClick={() => setTab("sell")} style={{ flex: 1, fontSize: 12, padding: 9 }}>💰 팔기</PxButton>
          </div>
          ...
          {tab === "buy" ? (
            <div>
              <div style={{ background: C.white, border: `3px solid ${C.ink}`, borderRadius: 10, padding: 11, marginBottom: 10 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13.5, fontWeight: "bold" }}>지렁이 미끼</div>
                    <div style={{ fontSize: 11, color: C.inkSoft }}>지금 {bait}개 · 한 번 던질 때 1개 써요</div>
                  </div>
                </div>
                <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                  {[[5, 4], [20, 14], [50, 30]].map(([n, p]) => (
                    <PxButton key={n} tone="gold" disabled={gold < p} onClick={() => onBuyBait(n, p)} style={{ flex: 1, fontSize: 11, padding: 8 }}>{n}개 🪙{p}</PxButton>
                  ))}
                </div>
              </div>
            ...
            <div>
              {items.length === 0 ? (
                <div style={{ fontSize: 12.5, color: C.inkSoft, textAlign: "center", padding: 26, lineHeight: 1.8 }}>팔 게 없네~ 🎣<br />낚시부터 하고 오게나</div>
              ) : (
                <>
                  <div style={{ display: "flex", flexDirection: "column", gap: 5, marginBottom: 10 }}>
                    {items.map((f) => (
                      <div key={f.id} style={{ display: "flex", alignItems: "center", gap: 9, background: C.white, border: `2px solid ${C.ink}`, borderRadius: 8, padding: "8px 10px" }}>
                        <span style={{ fontSize: 24 }}>{f.emoji}</span>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 13, fontWeight: "bold" }}>{f.name} ×{bag[f.id]}</div>
                  ...
                  <PxButton tone="good" onClick={onSellAll} style={{ width: "100%", padding: 12, fontSize: 14 }}>💰 전부 팔기 · 🪙{totalGold}</PxButton>
                  <div style={{ fontSize: 10.5, color: C.inkSoft, textAlign: "center", marginTop: 7 }}>🎁 비밀 상자는 열어보면 뜻밖의 선물이 나올지도?</div>
                </>
              )}
            </div>
          )}
...
function FishingView({ onBack, bubble, rod = 0, bait = 0, bag = {}, caught = {}, onCatch, onUseBait, onShop, onDex, onBreak }) {
  ...
  const cast = () => {
    if (bait <= 0) { setPhase("nobait"); return; }
    onUseBait();
    setGot(null); setPhase("wait");
    clearTimeout(timer.current);
    timer.current = setTimeout(() => {
      setPhase("bite");
      clearTimeout(timer.current);
      timer.current = setTimeout(() => setPhase("miss"), 1400);
  ...
  const furniture = [
    { id: "spot", x: 260, y: 140, w: 120, h: 120, color: "transparent", emoji: "🎣", label: "낚시 자리",
      onInteract: () => (phase === "wait" || phase === "bite" ? pull() : cast()) },
    { id: "fisher", x: 60, y: 230, w: 90, h: 90, color: "transparent", emoji: "🧔", label: "어부 아저씨", onInteract: () => onShop() },
    { id: "dex", x: 490, y: 230, w: 80, h: 80, color: "transparent", emoji: "📖", label: "바다 도감", onInteract: () => onDex() },
  ];

  return (
    <RoomView title="낚시터" icon="🎣" sub="입질이 오면 바로 당기세요 · 미끼가 있어야 던질 수 있어요" bg="#4fa8d8" roomW={640} roomH={400}
      furniture={furniture} onBack={onBack} paused bubble={bubble} headerBg="#1f5f96" tipId="fishing">
      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", padding: 14, zIndex: 20, background: "rgba(0,0,0,0.25)" }}>
        <div style={{ width: "100%", maxWidth: 390 }}>
          <Panel style={{ padding: 16, textAlign: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, justifyContent: "center", fontSize: 11.5, color: C.inkSoft, flexWrap: "wrap" }}>
              <span>{RODS[rod].emoji} {RODS[rod].name}</span><span>·</span><span>🪱 미끼 {bait}</span>
              <PxButton tone="blue" onClick={onDex} style={{ fontSize: 10.5, padding: "4px 9px" }}>📖 도감</PxButton>
              <PxButton tone="wood" onClick={onShop} style={{ fontSize: 10.5, padding: "4px 9px" }}>🧔 어부</PxButton>
            </div>

            <div style={{ height: 124, display: "flex", alignItems: "center", justifyContent: "center", margin: "10px 0" }}>
              {phase === "idle" && <div style={{ fontSize: 54 }}>🎣</div>}
              {phase === "nobait" && <div><div style={{ fontSize: 46 }}>🪱</div><div style={{ fontSize: 13.5, color: C.danger, fontWeight: "bold", marginTop: 8 }}>미끼가 없어요!</div><div style={{ fontSize: 11.5, color: C.inkSoft, marginTop: 3 }}>🧔 어부 아저씨에게 사오세요</div></div>}
              {phase === "wait" && <div><div className="sea-float" style={{ fontSize: 44 }}>🎣</div><div style={{ fontSize: 12.5, color: C.inkSoft, marginTop: 8 }}>찌를 보고 기다리는 중…</div></div>}
              {phase === "bite" && <div><div className="gift-pop" style={{ fontSize: 56 }}>❗</div><div style={{ fontSize: 15, color: C.danger, fontWeight: "bold", marginTop: 6 }}>입질이 왔어요! 당기세요!</div></div>}
              {phase === "result" && got && (
                <div>
                  <div className="gift-pop" style={{ fontSize: 56 }}>{got.emoji}</div>
                  <div style={{ fontSize: 15, fontWeight: "bold", marginTop: 6 }}>{got.name}</div>
                  <div style={{ fontSize: 12.5, marginTop: 3, color: got.type === "gem" ? "#2f7fb5" : got.type === "trash" ? C.inkSoft : C.good }}>
                    {got.type === "gem" ? (got.id === "diamond" ? "💎 젬 5 획득!" : "💎 젬 1 획득!")
                      : got.type === "box" ? "🎁 어부에게 열어달라고 해보세요"
                      : got.type === "trash" ? "…쓰레기지만 팔면 몇 골드는 돼요" : `팔면 🪙${got.sell}`}
                  </div>
                </div>
              )}
            ...
            <PxButton tone={phase === "bite" ? "danger" : "good"} disabled={bait <= 0 && phase !== "bite" && phase !== "wait"}
              onClick={() => (phase === "wait" || phase === "bite" ? pull() : cast())}
              style={{ width: "100%", padding: 14, fontSize: 15 }}>
              {phase === "bite" ? "🎣 당기기!" : phase === "wait" ? "기다리는 중… (누르면 헛챔질)" : bait <= 0 ? "🪱 미끼가 없어요" : `🎣 낚싯대 던지기 (🪱 -1)`}
            </PxButton>

            {log.length > 0 && (
              <div style={{ marginTop: 12 }}>
                <div style={{ fontSize: 11, color: C.inkSoft, marginBottom: 5 }}>🪣 방금 잡은 것</div>
                <div style={{ display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "center" }}>
                  {log.map((x) => <span key={x.key} title={x.name} style={{ fontSize: 20 }}>{x.emoji}</span>)}
                </div>
              </div>
            )}
...
/* ===== 🌊 바다 (별도 맵) ===== */
function SeaView({ pos, setPos, onBack, onFish, onShop, onDex, look = null, carry = null, pet = null, outfit = null, bubble = null, others = {} }) {
  const [facing, setFacing] = useState(1);
  const [moving, setMoving] = useState(false);
  const [near, setNear] = useState(null);
  const keys = useRef({});
  ...
  const SPOTS = [
    { id: "fish", x: SEA_PIER_X + SEA_PIER_W / 2, y: SEA_PIER_END - 30, r: 80, emoji: "🎣", label: "낚시터" },
    { id: "light", x: SEA_PIER_X - 150, y: SEA_PIER_END - 60, r: 70, emoji: "🗼", label: "등대" },
    { id: "shop", x: 420, y: SEA_WATER_Y - 46, r: 78, emoji: "🧔", label: "어부 아저씨" },
    { id: "dex", x: 1120, y: SEA_WATER_Y - 46, r: 74, emoji: "📖", label: "바다 도감" },
    { id: "back", x: SEA.w / 2, y: SEA_SAND_Y - 60, r: 90, emoji: "🚏", label: "마을로 돌아가기" },
  ];

  const act = (id) => {
    if (id === "fish") onFish && onFish();
    else if (id === "shop") onShop && onShop();
    else if (id === "dex") onDex && onDex();
    else if (id === "back") onBack && onBack();
  };
    ...
    const loop = (now) => {
      try {
        ...
        if (dx || dy) {
          const len = Math.hypot(dx, dy) || 1;
          x += (dx / len) * SPEED; y += (dy / len) * SPEED;
          x = Math.max(30, Math.min(SEA.w - 30, x));
          y = Math.max(60, Math.min(SEA.h - 30, y));
          const onPier = x > SEA_PIER_X + 8 && x < SEA_PIER_X + SEA_PIER_W - 8;
          if (y > SEA_WATER_Y - 14) {
            if (onPier) y = Math.min(y, SEA_PIER_END - 18);
            else y = SEA_WATER_Y - 14;
          }
          posRef.current = { x, y }; setPos({ x, y });
          if (dx < 0) setFacing(-1); else if (dx > 0) setFacing(1);
        }
        ...
        for (const sp of SPOTS) {
          ...
          if (d < sp.r && d < best) { best = d; found = sp.id; }
        }
        if (found !== nearRef.current) { nearRef.current = found; setNear(found); }
      } catch (err) { console.error("[SeaView] loop error:", err); } finally { raf = requestAnimationFrame(loop); }
    };
    raf = requestAnimationFrame(loop);
    return () => cancelAnimationFrame(raf);
  }, [setPos]);

  const camX = Math.max(0, Math.min(SEA.w - 640, pos.x - 320));
  const camY = Math.max(0, Math.min(SEA.h - 480, pos.y - 240));
  const here = Object.values(others).filter((o) => o.v === "sea");
        ...
        <div style={{ position: "absolute", width: SEA.w, height: SEA.h, left: -camX, top: -camY }}>
          ...
          {SPOTS.map((sp) => (
            <div key={sp.id} onClick={() => act(sp.id)} title={sp.label}
              style={{ position: "absolute", left: sp.x, top: sp.y, transform: "translate(-50%,-100%)", cursor: "pointer", textAlign: "center", zIndex: 8,
                filter: near === sp.id ? "drop-shadow(0 0 6px #ffcb2b)" : "drop-shadow(0 2px 1px rgba(0,0,0,0.3))" }}>
              <div style={{ fontSize: sp.id === "light" ? 54 : 44, lineHeight: 1 }}>{sp.emoji}</div>
              <span style={{ fontSize: 11, fontWeight: "bold", background: near === sp.id ? C.gem : "rgba(255,253,246,0.9)", border: `2px solid ${C.ink}`, borderRadius: 8, padding: "1px 7px", whiteSpace: "nowrap" }}>{sp.label}</span>
            </div>
          ))}

          {here.map((o) => (
            <div key={o.id} style={{ position: "absolute", left: o.x, top: o.y, transform: "translate(-50%,-100%)", zIndex: 9, transition: "left .18s linear, top .18s linear" }}>
              <div style={{ fontSize: 10, background: "#5b8def", color: C.white, border: `2px solid ${C.ink}`, padding: "0 5px", whiteSpace: "nowrap", textAlign: "center" }}>{o.name}</div>
              <Hero facing={o.f || 1} size={32} look={o.lk} pet={o.pt} carry={o.cy ? { emoji: o.cy } : null}
                outfit={o.oc ? { top: o.oc[0] ? { color: o.oc[0] } : null, bottom: o.oc[1] ? { color: o.oc[1] } : null, shoes: o.oc[2] ? { color: o.oc[2] } : null } : null} />
            </div>
          ))}

          <div style={{ position: "absolute", left: pos.x, top: pos.y, transform: "translate(-50%,-100%)", zIndex: 10 }}>
            {bubble && <div className="chat-bubble" style={{ position: "absolute", bottom: "112%", left: "50%", transform: "translateX(-50%)", background: C.white, border: `2px solid ${C.ink}`, padding: "3px 8px", fontSize: 12, whiteSpace: "nowrap" }}>{bubble}</div>}
...
/* 🏷 프로필 해시태그 (notices 재사용: type="ptag", title=대상 이름 · 누구나 달 수 있어요) */
async function dbLoadPtags(name) {
  try {
    const s = await getSupa();
    const r = await s.from("notices").select("body,created_at").eq("type", "ptag").eq("title", name).order("created_at", { ascending: false }).limit(1);
    const row = r && r.data && r.data[0];
    if (!row) return [];
...
async function dbSavePtags(name, tags) {
  try {
    const s = await getSupa();
    const r = await s.from("notices").insert({ type: "ptag", title: name, body: JSON.stringify(tags || []) });
    return !(r && r.error);
  } catch (e) { return false; }
}
...
function HomeView({ house, notes = [], onSaveMemo, onDelMemo, onBack, bubble, skin = null, extras = [], gifts = [], fridge = [], fishes = [], hasAquarium = false, hasYard = false, petsAtHome = [], onOpenAqua, onOpenYard, isMine = false, layout = {}, onMoveFurni, onResetLayout, onGiftHome, myName = "", watch = [], onWatchAdd, onWatchDel, onWatchReply, onWatchDelReply }) {
  ...
  const arrangeBar = isMine ? (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 12px", background: arrange ? C.gem : C.parch, borderBottom: `3px solid ${C.ink}`, flexWrap: "wrap" }}>
      <span style={{ fontSize: 12, flex: 1, minWidth: 120 }}>{arrange ? "🔧 가구를 끌어서 옮겨보세요" : "🏠 우리 집 · 가구 위치를 바꿀 수 있어요"}</span>
      {arrange && <PxButton tone="ink" onClick={() => { onResetLayout && onResetLayout(); }} style={{ fontSize: 11, padding: "5px 9px" }}>↩ 처음 배치로</PxButton>}
      <PxButton tone={arrange ? "danger" : "blue"} onClick={() => setArrange((v) => !v)} style={{ fontSize: 11, padding: "5px 10px" }}>
        {arrange ? "✓ 배치 끝내기" : "🔧 가구 배치"}
      </PxButton>
    </div>
  ) : null;
...
function questExp(q) {
  if (!q) return 0;
  if (q.exp != null && Number(q.exp) > 0) return Math.round(Number(q.exp));
  return Math.max(1, Math.round((Number(q.gem) || 0) * 3));
}
...
function BossMapView({ onBack, onReward, onGoSchool, onClearQuest, myName = "", accepted = {}, onAccept, onStart, onShout, onBoard, notes = {}, onNote, threads = {}, onThreadSend, onAgree, onLeave, maps = [], people = [], onAddQuest, onEditQuest, onDelQuest, onAddMap, onGoShrine, onSubmitAnswer, onGainExp, onCallParty, jumpTo = null, onJumpUsed, logs = {}, onLogAdd, onLogDel, onSyncParty, onClearedChange, onQuestDone }) {
  ...
  const [shots, setShots] = useState([]);            // 📷 인증 이미지 (최대 3장)
  const [imgBusy, setImgBusy] = useState(false);
  const [bigShot, setBigShot] = useState(null);      // 크게 보기
  const shotInputRef = useRef(null);
  const pickShots = async (files) => {
    ...
    try {
      ...
      for (const f of list) {
        ...
        try { out.push(await compressImage(f, 620, 0.62)); } catch (e) {}
      }
      setShots((v) => [...v, ...out].slice(0, 3));
    } finally { setImgBusy(false); if (shotInputRef.current) shotInputRef.current.value = ""; }
  };
  ...
  const [dexPick, setDexPick] = useState(null);   // 📖 도감 개별 항목 상세
  /* 퀘스트 창 안 화면 전환 : "party"(모집) ↔ "run"(진행) — 서로 자유롭게 오갈 수 있어요 */
  ...
  if (isPlaza) {
    ...
    openQuests.forEach((q, i) => {
      ...
      nodes.push({ ...q, exp: questExp(q), stage: st.n, stageName: st.name, x: Math.round(cx + Math.cos(ang) * rad * 0.72), y: Math.round(cy + Math.sin(ang) * rad * 0.92) });
    });
    nodes.push({ ...map.boss, exp: questExp(map.boss), stage: 99, stageName: "보스", isBoss: true, x: cx, y: cy });
  } else {
    map.stages.forEach((st, si) => {
      ...
      st.quests.forEach((q, qi) => {
        ...
        nodes.push({ ...q, exp: questExp(q), stage: st.n, stageName: st.name, x: 110 + col * 190, y: baseY + 130 + row * 120 });
      });
    });
    nodes.push({ ...map.boss, exp: questExp(map.boss), stage: map.stages.length, stageName: "보스", isBoss: true, x: MAP_W / 2, y: 150 });
  }
  ...
  useEffect(() => {
    ...
    /* 맵 id 를 몰라도(알림에서 눌렀을 때) 퀘스트 id 로 어느 맵인지 찾아요 */
    let i = jumpTo.mapId ? maps.findIndex((m) => m.id === jumpTo.mapId) : -1;
    if (i < 0) i = maps.findIndex((m) =>
      (m.boss && m.boss.id === jumpTo.qid) ||
      (m.stages || []).some((st) => (st.quests || []).some((q) => q.id === jumpTo.qid)));
  ...
  useEffect(() => {
    ...
    if (nd) { setSel(nd); setPendingQ(null); return; }
    if (jumpTick > 8) { setPendingQ(null); return; }   // 못 찾으면 포기
    const t = setTimeout(() => setJumpTick((v) => v + 1), 90);
    return () => clearTimeout(t);
  }, [pendingQ, mapIdx, jumpTick]);
  useEffect(() => {
    const down = (e) => {
      if (isTyping(e)) return;
  ...
  return (
    <Panel style={{ padding: 0, overflow: "hidden" }}>
              ...
              return (
                <div key={nd.id} style={{ position: "absolute", left: nd.x, top: nd.y, transform: "translate(-50%,-50%)", textAlign: "center", zIndex: 3 }}>
                  ...
                  <div style={{ marginTop: 5, fontSize: 11, fontWeight: "bold", color: nd.isBoss ? C.white : C.ink, background: nd.isBoss ? "rgba(0,0,0,0.45)" : "rgba(255,255,255,0.92)", borderRadius: 12, padding: "2px 9px", whiteSpace: "nowrap", boxShadow: "0 2px 3px rgba(0,0,0,0.18)" }}>
                    {accepted[nd.id] ? (accepted[nd.id].started ? "▶ " : "🤝 ") : ""}{nd.due && !done[nd.id] ? "⏳" : ""}{nd.level ? (nd.level === "초보자" ? "🌱" : "🔥") : ""}{nd.title}{nd.exp > 0 ? ` ⭐${nd.exp}` : ""}
                  </div>
                </div>
              );
            })}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 95, padding: 14 }} onClick={() => setDexOpen(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 520, maxHeight: "calc(100vh - 28px)", overflowY: "auto" }}>
            <div style={{ background: C.parch, border: `3px solid ${C.ink}`, borderRadius: 14, padding: 16, boxShadow: "0 10px 26px rgba(0,0,0,0.45)" }}>
                    ...
                    <div key={m.id}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
                        {qs.map((q) => {
                          ...
                          return (
                            <button key={q.id} type="button" onClick={() => setDexPick({ q, map: m, by, got })}
                              ...
                              <div style={{ fontSize: 10.5, marginTop: 4, fontWeight: "bold", color: got ? C.good : C.inkSoft, borderTop: `1px dashed ${C.parchEdge}`, paddingTop: 4 }}>
                                {got ? `✅ ${by || "누군가"} 완료` : "⏳ 아직 미완료"}
                              </div>
                              <div style={{ fontSize: 9.5, color: C.inkSoft, marginTop: 2 }}>눌러서 자세히 보기</div>
                              {got && (
                                <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(200,40,40,0.55)", fontSize: 52, fontWeight: "bold", lineHeight: 1, pointerEvents: "none", textShadow: "0 2px 3px rgba(255,255,255,0.7)" }}>✓</div>
                              )}
                            </button>
                          );
                        })}
                        <button type="button" onClick={() => setDexPick({ q: { ...m.boss, isBoss: true }, map: m, by: typeof dn[m.boss.id] === "string" ? dn[m.boss.id] : null, got: bossDone })}
                          ...
                          <div style={{ fontSize: 10.5, marginTop: 4, fontWeight: "bold", color: bossDone ? C.good : C.inkSoft, borderTop: `1px dashed ${C.parchEdge}`, paddingTop: 4 }}>
                            {bossDone ? `⚔ ${(typeof dn[m.boss.id] === "string" ? dn[m.boss.id] : "누군가")} 격파` : "⏳ 아직 미격파"}
                          </div>
                          <div style={{ fontSize: 9.5, color: C.inkSoft, marginTop: 2 }}>눌러서 자세히 보기</div>
                          {bossDone && <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(200,40,40,0.55)", fontSize: 52, fontWeight: "bold", lineHeight: 1, pointerEvents: "none", textShadow: "0 2px 3px rgba(255,255,255,0.7)" }}>✓</div>}
                        </button>
                      </div>
                    </div>
                  );
              ...
              <div style={{ fontSize: 12, color: C.inkSoft, marginTop: 12, textAlign: "center", lineHeight: 1.6 }}>도감을 눌러 자세한 내용과 완료한 사람을 확인하세요<br />완료한 건 초록 테두리와 ✓ 표시</div>
            </div>
          </div>
        </div>
      )}
      ...
      {dexPick && (
        <div onClick={() => setDexPick(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 120, padding: 14 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 380, maxHeight: "calc(100vh - 28px)", overflowY: "auto" }}>
            <div style={{ background: C.parch, border: `3px solid ${C.ink}`, borderRadius: 14, padding: 18, boxShadow: "0 10px 26px rgba(0,0,0,0.5)" }}>
              <div style={{ textAlign: "center" }}>
                <div style={{ width: 78, height: 78, margin: "0 auto", borderRadius: "50%", border: `3px solid ${C.ink}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 38,
                  background: dexPick.got ? `radial-gradient(circle at 35% 30%, #ffffff66, ${dexPick.map.color})` : "radial-gradient(circle at 35% 30%, #ffffff44, #9a94a6)" }}>
                  {dexPick.q.icon}
                </div>
                <div style={{ fontSize: 10.5, color: C.inkSoft, marginTop: 8 }}>{dexPick.map.icon} {dexPick.map.name}{dexPick.q.isBoss ? " · 👑 BOSS" : dexPick.q.stageName ? ` · ${dexPick.q.stageName}` : ""}</div>
                <div style={{ fontSize: 18, fontWeight: "bold", margin: "5px 0 8px" }}>{dexPick.q.title}</div>
              </div>

              {dexPick.q.desc && <div style={{ fontSize: 12.5, color: C.inkSoft, textAlign: "center", lineHeight: 1.7, marginBottom: 10 }}>{dexPick.q.desc}</div>}
              <div style={{ background: C.white, border: `2px solid ${C.ink}`, borderRadius: 10, padding: 12, fontSize: 13, lineHeight: 1.7 }}>🎯 {dexPick.q.task || "완료 조건 없음"}</div>

              <div style={{ display: "flex", gap: 7, marginTop: 10, flexWrap: "wrap" }}>
                <div style={{ flex: "1 1 120px", background: "#fff1d6", border: `2px solid ${C.ink}`, borderRadius: 8, padding: "8px 10px", textAlign: "center" }}>
                  <div style={{ fontSize: 10, color: C.inkSoft }}>⭐ 경험치</div>
                  <b style={{ fontSize: 15, color: C.danger }}>+{questExp(dexPick.q)}</b>
                </div>
                <div style={{ flex: "1 1 120px", background: "#fff1d6", border: `2px solid ${C.ink}`, borderRadius: 8, padding: "8px 10px", textAlign: "center" }}>
                  <div style={{ fontSize: 10, color: C.inkSoft }}>🎁 보상</div>
                  <b style={{ fontSize: 12.5, color: "#a86e13" }}>{rewardLabel(dexPick.q) || "없음"}</b>
                </div>
              </div>

              {/* ✅ 완료한 사람 */}
              <div style={{ marginTop: 10, background: dexPick.got ? "#eef6ef" : "#f3efe4", border: `2px solid ${dexPick.got ? C.good : C.parchEdge}`, borderRadius: 10, padding: 12 }}>
                <div style={{ fontSize: 11, fontWeight: "bold", color: C.inkSoft, marginBottom: 6 }}>{dexPick.q.isBoss ? "⚔ 격파한 사람" : "✅ 완료한 사람"}</div>
                {dexPick.got ? (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 13, fontWeight: "bold", color: C.white, background: C.good, border: `2px solid ${C.ink}`, borderRadius: 14, padding: "4px 12px" }}>
                      🧑 {dexPick.by || "익명의 주민"}
                    </span>
                    {dexPick.by === myName && <span style={{ fontSize: 11, color: C.good, fontWeight: "bold" }}>(나예요!)</span>}
                  </div>
                ) : (
                  <div style={{ fontSize: 12.5, color: C.inkSoft, lineHeight: 1.6 }}>아직 아무도 완료하지 않았어요 🔒<br />가장 먼저 도전해보세요!</div>
                )}
              </div>

              {/* 참가자 · 등록 정보 */}
              <div style={{ marginTop: 10, fontSize: 11.5, color: C.inkSoft, lineHeight: 1.9, background: C.white, border: `2px solid ${C.parchEdge}`, borderRadius: 8, padding: "9px 11px" }}>
                {dexPick.q.owner ? <>✍️ 만든 사람 : <b style={{ color: C.ink }}>{dexPick.q.owner}</b><br /></> : <>🏛 기본 퀘스트<br /></>}
                {dexPick.q.registrar && <>📋 검토 담당 : <b style={{ color: C.ink }}>{dexPick.q.registrar}</b><br /></>}
                👥 참가 : {(!dexPick.q.who || dexPick.q.who === "all" || !Array.isArray(dexPick.q.who) || !dexPick.q.who.length) ? "모두 참가 가능" : dexPick.q.who.join(", ")}
                {dexPick.q.due && <><br />⏳ 마감 : {new Date(dexPick.q.due).toLocaleString("ko-KR", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" })}</>}
                {dexPick.q.level && <><br />🎚 난이도 : {dexPick.q.level}</>}
              </div>

              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                <PxButton tone="ink" onClick={() => setDexPick(null)} style={{ flex: 1, padding: 11, fontSize: 13 }}>닫기</PxButton>
                {!dexPick.got && !dexPick.q.isBoss && (
                  <PxButton tone="gold" onClick={() => {
                    const nd = nodesRef.current.find((x) => x.id === dexPick.q.id);
                    setDexPick(null); setDexOpen(false);
                    if (nd) setSel(nd);
                  }} style={{ flex: 1, padding: 11, fontSize: 13 }}>🎯 도전하러 가기</PxButton>
                )}
              </div>
            </div>
          </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.68)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 14 }} onClick={() => setSubmitFor(null)}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 380, maxHeight: "calc(100vh - 28px)", overflowY: "auto" }}>
            <Panel style={{ padding: 16 }}>
              ...
              {/* 📷 인증 이미지 첨부 */}
              <div style={{ fontSize: 11.5, fontWeight: "bold", margin: "12px 0 5px" }}>📷 인증 이미지 (선택 · 최대 3장)</div>
              <input ref={shotInputRef} type="file" accept="image/*" multiple onChange={(e) => pickShots(e.target.files)} style={{ display: "none" }} />
              <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                {shots.map((src, i) => (
                  <div key={i} style={{ position: "relative", width: 78, height: 78, border: `2px solid ${C.ink}`, borderRadius: 8, overflow: "hidden", background: "#000" }}>
                    <img src={src} alt="" onClick={() => setBigShot(src)} style={{ width: "100%", height: "100%", objectFit: "cover", cursor: "zoom-in", display: "block" }} />
                    <button type="button" onClick={() => setShots((v) => v.filter((_, x) => x !== i))}
                      style={{ position: "absolute", right: 2, top: 2, width: 20, height: 20, borderRadius: "50%", border: `2px solid ${C.ink}`, background: C.danger, color: C.white, cursor: "pointer", fontSize: 11, lineHeight: 1, padding: 0 }}>✕</button>
                  </div>
                ))}
                {shots.length < 3 && (
              ...
              <div style={{ fontSize: 10, color: C.inkSoft, marginTop: 5, lineHeight: 1.6 }}>사진은 자동으로 용량을 줄여서 제단에 함께 올라가요</div>

              <PxButton tone="gold" disabled={!answer.trim()} onClick={() => {
                const q = submitFor;
                clear(q);
                const imgs = shots;
                setSubmitFor(null); setSel(null); setShots([]);
            ...
            <div style={{ background: C.parch, border: `3px solid ${C.ink}`, borderRadius: 14, padding: 16 }}>
              ...
              {addTab === "quest" ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                  ...
                  <div style={{ fontSize: 11, fontWeight: "bold", color: C.danger }}>⭐ 경험치 (필수)</div>
                  <div style={{ background: C.white, border: `2px solid ${Number(fQ.exp) > 0 ? C.ink : C.danger}`, borderRadius: 8, padding: 9 }}>
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      <input value={fQ.exp} onChange={(e) => setFQ({ ...fQ, exp: e.target.value })} type="number" min="1" placeholder="10"
                        style={{ width: 84, padding: 8, border: `2px solid ${C.ink}`, borderRadius: 6, fontSize: 14, fontWeight: "bold" }} />
                      <span style={{ flex: 1, fontSize: 10.5, color: C.inkSoft, lineHeight: 1.5 }}>완료하면 바로 들어오는 경험치예요 (레벨업에 쌓여요)</span>
                    </div>
                    ...
                    {!(Number(fQ.exp) > 0) && <div style={{ fontSize: 10.5, color: C.danger, marginTop: 6, fontWeight: "bold" }}>경험치는 1 이상으로 꼭 정해주세요</div>}
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                    <PxButton tone="ink" onClick={() => setAddOpen(false)} style={{ flex: 1, padding: 10, fontSize: 13 }}>취소</PxButton>
                    <PxButton tone="gold" disabled={!fQ.title.trim() || !(Number(fQ.exp) > 0) || (fQ.regMode === "other" && !fQ.regName)} onClick={addQuest} style={{ flex: 1, padding: 10, fontSize: 13 }}>퀘스트 추가</PxButton>
                  </div>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
      ...
      {sel && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.62)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 90, padding: 14 }} onClick={() => setSel(null)}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 370, maxHeight: "calc(100vh - 28px)", overflowY: "auto", WebkitOverflowScrolling: "touch" }}>
            <div style={{ background: C.parch, border: `3px solid ${C.ink}`, borderRadius: 14, padding: 18, boxShadow: "0 10px 26px rgba(0,0,0,0.45)" }}>
              ...
              {sel.exp > 0 && (
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 7, background: "#fff1d6", border: `2px solid ${C.ink}`, borderRadius: 8, padding: "7px 10px", marginBottom: 10 }}>
                  <span style={{ fontSize: 16 }}>⭐</span>
                  <b style={{ fontSize: 13, color: C.danger }}>경험치 +{sel.exp}</b>
                    ...
                    {acc && acc.locked && !isMember ? (
                      ...
                      <div>
                        ...
                        {!isMember ? (
                          ...
                          canJoin(sel) ? (
                            <>
                              <PxButton tone="gold" onClick={() => { onAccept && onAccept(sel.id, sel.title, map.id); setQView("party"); }} style={{ width: "100%", padding: 11, fontSize: 13 }}>🤝 퀘스트 수락하기</PxButton>
                          ...
                          <>
                            ...
                            {started ? (
                              ...
                              <PxButton tone="good" onClick={() => { onStart && onStart(sel.id, map.id); setQView("run"); }} style={{ width: "100%", padding: 10, fontSize: 13 }}>▶ 퀘스트 시작</PxButton>
                            )}
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <PxButton tone="gold" disabled={!!done[sel.id] || !!lockReason(sel)}
                  onClick={() => { const q = sel; if (q.isBoss) { clear(q); setSel(null); } else { setSubmitFor(q); setAnswer(""); setShots([]); } }}
      ...
      {bigShot && (
        <div onClick={() => setBigShot(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.88)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 16, cursor: "zoom-out" }}>
          <img src={bigShot} alt="" style={{ maxWidth: "100%", maxHeight: "100%", border: `3px solid ${C.white}`, borderRadius: 8 }} />
        </div>
      )}
    </Panel>
  );
...
/* ======================= 게시판(캘린더 + 공지) ======================= */
const UPDATE_NOTES = [
  { id: "u20260728n8", type: "업데이트", date: "2026-07-28", title: "💬 지식인 최신글 방 개편",
    body: "· 상단에 제품 버튼(좌측) · 📚 예시 · 💬 답변 · ⚙️ 설정(관리자)을 넣었어요\n· ⚙️ 설정에서 제품·키워드를 한 줄에 하나씩 일괄 등록해요 (카페 방과 동일)\n· 크롤링된 지식인 링크를 한 줄에 하나씩 나열하고, 우측에 답변불가 / 답변완료 버튼을 넣었어요\n· 💬 답변에서 답변완료 / 답변불가 탭으로 알바가 누른 링크를 최신순으로 봐요\n· 📚 예시는 튜토리얼 게시판이에요 — 모두가 보고, 글쓰기는 관리자만 (제목+내용, 서버 저장)\n· 1시간 목표 50개 타이머 유지 · 답변완료를 누르면 카운트가 올라가요\n· 크롤링 링크는 몰입의방 API 연결 시 자동으로 채워져요" },
  ...
  { id: "u20260724n19", type: "업데이트", date: "2026-07-24", title: "🔔 카테고리별 알림 숫자 · ✉️ 확인하면 사라지는 메세지함",
    body: "· ✉️ 메세지함·🗺 퀘스트함 안의 탭마다 새로 생긴 개수가 빨간 숫자로 떠요\n· ✉️ 메세지함은 「✓ 확인」을 누르면 그 항목이 사라져요 (「✓ 전체 확인」도 있어요)\n· 확인하기 전까지는 열어봐도 숫자가 남아 있어요\n· 🗺 퀘스트함은 확인해도 사라지지 않아요 — 보스맵 도전기와 실시간으로 연동되는 목록이에요\n· 퀘스트함을 열 때마다 완료 현황을 서버에서 다시 읽어와 최신 상태로 맞춰요\n· [수정] 퀘스트함에서 「이동」을 눌러도 넘어가지 않던 문제를 고쳤어요 (맵 정보가 없어도 퀘스트 id 로 찾아갑니다)" },
  ...
  { id: "u20260724qq", type: "업데이트", date: "2026-07-24", title: "🌊 남쪽 바다 · 🎣 낚시터 오픈",
    body: "· 십자로 세로길을 따라 남쪽 끝까지 내려가면 모래사장과 바다가 나와요\n· 파도가 밀려오고 ⛵ 배와 🐬 돌고래가 떠다녀요\n· 바다는 헤엄칠 수 없고, 길 끝 선착장 위로만 걸어갈 수 있어요\n· 선착장 끝에 🎣 낚시터가 생겼어요 — 던지고 ❗ 입질이 오면 바로 당기세요\n· 🐟 🐠 🦐 🦑 🐡 🦀 🐙 … 잡은 만큼 🪙 골드를 받아요 (가끔 낡은 장화도 나와요)\n· 🗼 등대와 ⛱️ 파라솔 등 해변 소품도 놓았어요" },
  { id: "u20260724pp", type: "업데이트", date: "2026-07-24", title: "🪑 가구를 아이콘으로 · 🔧 배치 바꾸기",
    body: "· 가구가 네모 상자에서 아이콘으로 바뀌어 방이 훨씬 깔끔해졌어요\n· 이름표는 작은 알약 모양이고, 가까이 가면 노랗게 강조돼요\n· 🔧 내 집에서 「가구 배치」를 켜면 가구를 끌어서 옮길 수 있어요\n· 옮긴 위치는 저장되고 「↩ 처음 배치로」로 되돌릴 수 있어요\n· 마우스·터치 모두 지원해요" },
...
const HELP_DATA = {
  ...
  "🎮 놀이": [
    { icon: "🌊", title: "바다 가는 법", body: "십자로 세로길을 따라 남쪽 끝까지 내려가면 🌊 바다 가는 길이 있어요. 들어가면 마을과는 다른 바다 지역으로 이동합니다. 돌아올 땐 바다 위쪽 🚏 정류장을 누르세요." },
    { icon: "🎣", title: "낚시터", body: "바다 선착장 끝에 있어요. 던지려면 🪱 미끼가 1개씩 필요하고, ❗ 입질이 오면 1.4초 안에 당기세요. 🐟 물고기 · 🗑 쓰레기 · 💎 젬 · 💠 다이아 · 🎁 비밀 상자가 나와요." },
    { icon: "🧔", title: "어부 아저씨", body: "바다 모래사장에 있어요. 🪱 지렁이 미끼와 🎣 낚싯대를 팔고, 잡은 물고기·쓰레기·비밀 상자를 사줍니다. 낚싯대를 업그레이드하면 귀한 게 훨씬 잘 걸려요." },
    { icon: "📖", title: "바다 도감", body: "바다에서만 볼 수 있어요. 무엇이 잡히는지, 확률과 파는 값이 얼마인지 표로 보여줘요. 아직 못 잡은 건 ❔ 로 가려져 있어요." },
...
function Sheet({ icon, title, onClose, tabs, tab, setTab, maxW = 470, children, embed = false }) {
  ...
  /* 허브 안에 끼워넣을 때는 배경·패널 없이 내용만 */
  if (embed) return <div>{inner(false)}</div>;
  return (
        ...
        <div>
          ...
          <div style={{ background: C.white, border: `3px solid ${C.ink}`, borderRadius: 10, padding: 12, marginBottom: 10 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
              ...
              <PxButton tone={editOpen ? "good" : "blue"} onClick={() => setEditOpen((v) => !v)} style={{ fontSize: 11, padding: "6px 10px" }}>{editOpen ? "💾 저장" : "✏️ 꾸미기"}</PxButton>
            </div>
            ...
            {editOpen && (
              <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                ...
                <PxButton tone="good" onClick={() => setEditOpen(false)} style={{ width: "100%", padding: 12, fontSize: 13.5 }}>💾 저장하기</PxButton>
                <div style={{ fontSize: 10.5, color: C.inkSoft, textAlign: "center", marginTop: -3 }}>바꾼 내용은 바로 반영되고 자동으로 저장돼요</div>
              </div>
            )}
          </div>
...
function MessageCenter({ onClose, myName, box, notices, onReadAll, onAnswerInvite, onOpenDm, onCallBack, onGoMeeting, onDone, onDoneNotice, onDoneAll, embed = false }) {
  const [tab, setTab] = useState("notice");
  const empty = (t) => <div style={{ fontSize: 12, color: C.inkSoft, textAlign: "center", padding: 26, lineHeight: 1.8 }}>{t}</div>;
  /* 「✓ 확인」을 누르기 전까지 남아 있어요 — 그래서 개수 = 아직 확인 안 한 것 */
  const unread = (k) => (box[k] || []).length;
  const nUnreadNotice = notices.filter((n) => !n.read).length;
  const chk = (fn) => (
    <PxButton tone="good" onClick={fn} style={{ fontSize: 10, padding: "5px 7px" }}>✓ 확인</PxButton>
  );
  const curCount = tab === "notice" ? nUnreadNotice : unread(tab);
  return (
    <Sheet icon="✉️" title="메세지함" onClose={onClose} tab={tab} setTab={setTab} embed={embed}
      tabs={[
      ...
      {curCount > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 8 }}>
          <span style={{ flex: 1, fontSize: 11, color: C.inkSoft }}>확인하지 않은 항목 {curCount}개 · 확인하면 목록에서 사라져요</span>
          <PxButton tone="ink" onClick={() => onDoneAll && onDoneAll(tab)} style={{ fontSize: 10.5, padding: "6px 10px", whiteSpace: "nowrap" }}>✓ 전체 확인</PxButton>
        </div>
      )}
      <div style={{ maxHeight: 350, overflow: "auto", display: "flex", flexDirection: "column", gap: 7 }}>
        {tab === "notice" && (notices.filter((n) => !n.read).length === 0 ? empty("새로운 공지사항이 없어요 📭") : notices.filter((n) => !n.read).map((n) => (
          <MsgRow key={n.id} icon={n.type === "공지" ? "📢" : n.type === "모집" ? "🤝" : "🛠"} title={`[${n.type}] ${n.title}`} body={n.body} at={n.date} unread
            right={chk(() => onDoneNotice && onDoneNotice(n.id))} />
        )))}
          ...
          <MsgRow key={m.id} icon="📨" unread={!m.read}
            ...
            right={
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                ...
                {m.answered ? chk(() => onDone && onDone("invite", m.id)) : (
                  <>
        ...
        {tab === "gift" && ((box.gift || []).length === 0 ? empty("받은 선물이나 편지가 없어요 📭") : (box.gift || []).map((m) => (
          <MsgRow key={m.id} icon={m.item ? "🎁" : "💌"} unread={!m.read}
            ...
            right={chk(() => onDone && onDone("gift", m.id))} />
        )))}
        ...
        {tab === "dm" && ((box.dm || []).length === 0 ? empty("받은 메세지가 없어요 📭\n메뉴 → 마을주민들에서 DM을 보낼 수 있어요!") : (box.dm || []).map((m) => (
          <MsgRow key={m.id} icon="💬" unread={!m.read} title={`${m.from}님의 메세지`} body={m.text} at={m.at}
            right={<div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              <PxButton tone="blue" onClick={() => onOpenDm(m.from)} style={{ fontSize: 10, padding: "5px 7px" }}>답장</PxButton>
              {chk(() => onDone && onDone("dm", m.id))}
            </div>} />
        )))}
        ...
        {tab === "call" && ((box.call || []).length === 0 ? empty("부재중 통화가 없어요 📞") : (box.call || []).map((m) => (
          <MsgRow key={m.id} icon="📵" unread={!m.read} title={`${m.from}님의 부재중 통화`} body={m.reason || "받지 못한 통화예요"} at={m.at}
            right={<div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              <PxButton tone="good" onClick={() => onCallBack(m.from)} style={{ fontSize: 10, padding: "5px 7px" }}>📞 콜백</PxButton>
              {chk(() => onDone && onDone("call", m.id))}
            </div>} />
        )))}
      </div>
    </Sheet>
...
function InventorySheet({ onClose, gold, outfit, ownedClothes, ikeaOwned, houseSkin, vehicle, myFurni, thanksInv, onEquipCloth, onToggleIkea, carrying, onGiftAct, skills = [], stats = {} }) {
  ...
  const [pick, setPick] = useState(null);
  const [skPick, setSkPick] = useState(null);
    ...
    <Sheet icon="🎒" title="인벤토리" onClose={onClose} tab={tab} setTab={setTab}
        ...
        <div>
            ...
            {SPECIAL_ITEMS.map((it) => {
              ...
              return (
                <button key={it.id} type="button" onClick={() => setPick(it)}
                  ...
                  <div style={{ fontSize: 10, marginTop: 3, color: have ? it.color : C.inkSoft, fontWeight: "bold" }}>{have ? it.rarity : "미발견"}</div>
                  ...
                  <div style={{ fontSize: 9.5, color: C.inkSoft, marginTop: 4 }}>{have ? "눌러서 설명 보기" : progress(it.id)}</div>
                </button>
              );
            })}
          </div>
          {pick && (
            <div onClick={() => setPick(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 140, padding: 14 }}>
              <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 320 }}>
                <div style={{ background: C.parch, border: `4px solid ${C.ink}`, borderRadius: 14, padding: 20, textAlign: "center" }}>
                  <div style={{ fontSize: 54, filter: owns(pick.id) ? "none" : "grayscale(1)", opacity: owns(pick.id) ? 1 : 0.5 }}>{owns(pick.id) ? pick.emoji : "❓"}</div>
                  <div style={{ fontSize: 11, color: owns(pick.id) ? pick.color : C.inkSoft, fontWeight: "bold", marginTop: 8 }}>{owns(pick.id) ? pick.rarity : "미발견 아이템"}</div>
                  <div style={{ fontSize: 17, fontWeight: "bold", margin: "3px 0 8px" }}>{owns(pick.id) ? pick.name : "??? "}</div>
                  <div style={{ fontSize: 12.5, color: C.inkSoft, lineHeight: 1.8 }}>{owns(pick.id) ? pick.desc : "아직 만나지 못한 아이템이에요. 조건을 채우면 이름과 설명이 열려요."}</div>
                  <div style={{ background: C.white, border: `2px dashed ${C.ink}`, borderRadius: 8, padding: 10, fontSize: 12, lineHeight: 1.7, margin: "12px 0" }}>
      ...
      {tab === "skill" && (
        <div>
          ...
          {skPick && (
            <div onClick={() => setSkPick(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 140, padding: 14 }}>
              <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 310 }}>
                <div style={{ background: C.parch, border: `4px solid ${C.ink}`, borderRadius: 14, padding: 20, textAlign: "center" }}>
                  <div style={{ fontSize: 50, filter: skPick.have ? "none" : "grayscale(1)", opacity: skPick.have ? 1 : 0.45 }}>{skPick.have ? skPick.icon : "❓"}</div>
                  <div style={{ fontSize: 17, fontWeight: "bold", margin: "8px 0 6px" }}>{skPick.have ? skPick.name : "???"}</div>
                  <div style={{ fontSize: 12.5, color: C.inkSoft, lineHeight: 1.8 }}>
                    {skPick.have ? skPick.desc : "아직 배우지 않은 스킬이에요."}
                  </div>
                  <div style={{ background: C.white, border: `2px dashed ${C.ink}`, borderRadius: 8, padding: 10, fontSize: 12, lineHeight: 1.7, margin: "12px 0" }}>
                    📌 배우는 방법<br /><b>🗺 보스맵 하드모드 퀘스트 완료</b>
                  </div>
                  <div style={{ fontSize: 13, fontWeight: "bold", color: skPick.have ? C.good : C.danger, marginBottom: 12 }}>
                    {skPick.have ? "✔ 배운 스킬이에요" : "🔒 아직 배우지 않았어요"}
                  </div>
                  <PxButton tone="ink" onClick={() => setSkPick(null)} style={{ width: "100%", padding: 11, fontSize: 13 }}>닫기</PxButton>
                </div>
              </div>
            </div>
          )}
...
function HQSidePanel({ myName = "", people = [], questBox = [], onOpenHQ }) {
  ...
  const [pOther, setPOther] = useState(null);
  const [pTags, setPTags] = useState([]);
  const [pTagIn, setPTagIn] = useState("");
  const pAccts = Array.from(new Set([myName, ...(people || []).map((p) => p && p.name)].filter(Boolean)));
  useEffect(() => {
    let alive = true;
    if (!pView) { setPOther(null); setPTags([]); return; }
    if (pView !== myName) { dbLoadMyPage(pView).then((d) => { if (alive) setPOther(d && typeof d === "object" ? d : {}); }); }
    else { setPOther(null); }
    dbLoadPtags(pView).then((t) => { if (alive) setPTags(Array.isArray(t) ? t : []); });
    return () => { alive = false; };
  }, [pView, myName]);
  ...
  const pAv = (pView || "?").trim().slice(0, 1);
  let pH = 0; for (let i = 0; i < (pView || "").length; i++) pH = (pH * 31 + pView.charCodeAt(i)) % 360;
  const pColor = `hsl(${pH},55%,58%)`;
        ...
        <div style={cardBox}>
          ...
          {MISSIONS.map((m, i) => (
            <label key={i} style={{ display: "flex", alignItems: "flex-start", gap: 6, fontSize: 10.5, marginBottom: 6, cursor: pSelf ? "pointer" : "default", lineHeight: 1.5 }}>
              ...
              <span style={{ textDecoration: vDone[i] ? "line-through" : "none", color: vDone[i] ? C.inkSoft : C.ink }}>{m}</span>
            </label>
          ))}
        </div>
...
function HQView({ onClose, myName = "", people = [], notices = [], onPostNotice, bossMaps = [], bossCleared = {}, qAccept = {}, questBox = [], unreadMsgCount = 0, onGo, onGoBoard, hqQuests = [], onHQChange, hqRoad = {}, onRoadChange }) {
  ...
  const [mpOther, setMpOther] = useState(null);   // 남의 프로필 데이터 { mission, todos } | null
  const [mpTags, setMpTags] = useState([]);       // 지금 보는 프로필의 해시태그
  const [mpTagIn, setMpTagIn] = useState("");
  const mpAccts = Array.from(new Set([myName, ...(people || []).map((p) => p && p.name)].filter(Boolean)));
  useEffect(() => {   // 보는 계정이 바뀌면 서버에서 불러와요 (실패해도 안 죽게 방어적)
    let alive = true;
    if (!mpView) { setMpOther(null); setMpTags([]); return; }
    if (mpView !== myName) { dbLoadMyPage(mpView).then((d) => { if (alive) setMpOther(d && typeof d === "object" ? d : {}); }); }
    else { setMpOther(null); }
    dbLoadPtags(mpView).then((t) => { if (alive) setMpTags(Array.isArray(t) ? t : []); });
    return () => { alive = false; };
  }, [mpView, myName]);
  const mpAddTag = () => {
    const t = mpTagIn.trim().replace(/^#/, ""); if (!t || !mpView) return;
    const next = Array.from(new Set([...(mpTags || []), t])).slice(0, 30);
              ...
              <div style={card}>
                ...
                {MP_MISSIONS.map((m, i) => (
                  <label key={i} style={{ display: "flex", alignItems: "flex-start", gap: 8, fontSize: 12.5, marginBottom: 8, cursor: mpSelf ? "pointer" : "default", lineHeight: 1.5 }}>
                    ...
                    <span style={{ textDecoration: viewDone[i] ? "line-through" : "none", color: viewDone[i] ? C.inkSoft : C.ink }}>{m}</span>
                  </label>
                ))}
              </div>
...
/* ======================= 🗺 퀘스트함 ======================= */
const QBOX_ICON = { new: "🆕", join: "🤝", start: "▶", done: "🏆", leave: "🚪", call: "📣", log: "📓", reward: "🎁", shrine: "✦" };
const QBOX_TEXT = {
...
function QuestBoxSheet({ onClose, rows = [], onRead, onClear, onDoneRow, onClaim, maps = [], accept = {}, cleared = {}, myName = "", onGo, embed = false }) {
  ...
  return (
    <Sheet title="🗺 퀘스트함" onClose={onClose} embed={embed}>
...
function QuestFragmentInput({ tone, icon, title, hint, placeholder, value, onChange, detail, onDetail, onAdd, imgs = [], onImgs }) {
  ...
  const pick = async (files) => {
    const list = Array.from(files || []).slice(0, 3);
    if (!list.length || !onImgs) return;
    setBusy(true);
    try {
      const out = [];
      for (const f of list) { if (!/^image\//.test(f.type)) continue; try { out.push(await compressImage(f, 620, 0.62)); } catch (e) {} }
      onImgs([...(imgs || []), ...out].slice(0, 3));
      ...
      {onImgs && (
        <>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 7 }}>
            {(imgs || []).map((src, i) => (
              <div key={i} style={{ position: "relative", width: 58, height: 58, border: `2px solid ${line}`, borderRadius: 6, overflow: "hidden" }}>
                <img src={src} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                <button type="button" onClick={() => onImgs((imgs || []).filter((_, x) => x !== i))}
                  style={{ position: "absolute", right: 1, top: 1, width: 18, height: 18, borderRadius: "50%", border: "none", background: "rgba(192,86,58,0.95)", color: "#fff", cursor: "pointer", fontSize: 10, lineHeight: 1, padding: 0 }}>✕</button>
              </div>
            ))}
            {(imgs || []).length < 3 && (
...
function QuestDoneView({ myName = "", onBack, bubble, draft = null, onDraftUsed, items = [], onAdd, onToggle, onDelete }) {
  ...
  const [reqI, setReqI] = useState([]); const [accI, setAccI] = useState([]);
  const [zoom, setZoom] = useState(null);   // 🖼 이미지 크게 보기
  const [filter, setFilter] = useState("all");
  const [q, setQ] = useState("");
  const [toast, setToast] = useState(null);
  ...
  return (
    <Panel style={{ padding: 0, overflow: "hidden" }}>
      ...
      <div style={{ padding: 16, background: "radial-gradient(circle at 50% 0%, #3a2e6b 0%, #1a1436 60%, #120e28 100%)" }}>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {(it.imgs || []).length > 0 && (
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 7 }}>
                        {it.imgs.map((src, ii) => (
                          <img key={ii} src={src} alt="인증 이미지" onClick={() => setZoom(src)}
                            style={{ width: 72, height: 72, objectFit: "cover", border: `2px solid ${line}`, borderRadius: 6, cursor: "zoom-in", display: "block" }} />
                        ))}
                      </div>
                    )}
        ...
        {zoom && (
          <div onClick={() => setZoom(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 16, cursor: "zoom-out" }}>
            <img src={zoom} alt="" style={{ maxWidth: "100%", maxHeight: "100%", border: "3px solid #ffd75e", borderRadius: 8 }} />
          </div>
        )}
        {toast && (
...
function EchoTown() {
  ...
  const [seaPos, setSeaPos] = useState({ x: SEA.w / 2, y: SEA_SAND_Y + 40 });
  /* 🎣 낚시 : 낚싯대 · 미끼 · 잡은 것 · 도감 */
  ...
  const doCatch = (f) => {
    ...
    bump("fish");
    if (f.type === "gem") {
      const n = f.id === "diamond" ? 5 : 1;
      award(n);
      showNotice(`${f.emoji} ${f.name}! 💎 젬 ${n} 획득`);
      return;
    }
    setCatchBag((v) => ({ ...v, [f.id]: (v[f.id] || 0) + 1 }));
  };
  const sellOne = (id) => {
    const f = SEA_CATCH.find((x) => x.id === id);
    const n = catchBag[id] || 0;
    if (!f || !n) return;
    awardGold(f.sell * n);
    setCatchBag((v) => { const c = { ...v }; delete c[id]; return c; });
    showNotice(`💰 ${f.name} ${n}개를 팔아 🪙${f.sell * n} 받았어요`);
  };
  const sellAll = () => {
    let sum = 0;
    SEA_CATCH.forEach((f) => { if (f.type !== "gem") sum += f.sell * (catchBag[f.id] || 0); });
    if (!sum) return;
    awardGold(sum);
    setCatchBag((v) => { const c = { ...v }; SEA_CATCH.forEach((f) => { if (f.type !== "gem") delete c[f.id]; }); return c; });
    showNotice(`💰 전부 팔아 🪙${sum} 받았어요`);
  };
  const openBox = () => {
  ...
  const addShrine = (row) => {
    ...
    noticeShrine(it);
    if (netSendEvent) {
      // 사진이 너무 크면 전송이 실패할 수 있어서, 큰 경우엔 사진 없이 공유해요
      let out = it;
      try { if (JSON.stringify(it).length > 150000) out = { ...it, imgs: [], detail: (it.detail || "") + "\n(📷 사진은 용량이 커서 올린 사람 기기에만 남아요)" }; } catch (e) { out = { ...it, imgs: [] }; }
      netSendEvent("shr", { row: out });
    }
  };
  ...
  useEffect(() => { if (view === "sea") netPosRef.current = seaPos; }, [seaPos, view]);
  ...
  /* 🏠 내 집 꾸민 모습을 다른 사람도 볼 수 있게 공유해요 */
  const DECOR_KEY = "echotown_housedecor_v1";
  const [houseDecor, setHouseDecor] = useState(() => loadJSON(DECOR_KEY, null) || {});
  const decorRef = useRef({});
  decorRef.current = houseDecor;
  const myHouseId = useMemo(() => {
  ...
  /* 내 집 꾸밈이 바뀌면 다른 사람에게도 알려줘요 (사진 없는 가벼운 정보만) */
  const myDecor = useMemo(() => ({
    skin: houseSkin ? houseSkin.id : null,
    extras: myFurni || [],
    layout: homeLayout || {},
    aqua: (facilities || []).includes("aquarium"),
    yard: (facilities || []).includes("yard"),
    fishes: fishes || [],
    pets: (PETS || []).filter((x) => (pets || []).includes(x.id) && x.id !== activePet).map((x) => `${x.emoji} ${x.name}`),
    gifts: (homeGifts || []).map((g) => ({ id: g.id, name: g.name, emoji: g.emoji, from: g.from })),
  ...
  const noticeShrine = useCallback((rows) => {
    ...
    const list = (Array.isArray(rows) ? rows : [rows]).filter((r) =>
      r && r.reviewer === me && r.who !== me && (r.kind || "acc") === "acc");
    if (!list.length) return;
    setQuestBox((v) => {
      const have = new Set(v.map((x) => x.shrId).filter(Boolean));
      const add = list.filter((r) => !have.has(r.id)).map((r) => ({
        id: Date.now() + Math.random(), read: false, kind: "shrine", shrId: r.id, goto: "shrine",
        who: r.who, title: r.text || "퀘스트",
        at: new Date().toLocaleString("ko-KR", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" }),
      }));
      if (!add.length) return v;
      const next = [...add, ...v].slice(0, 60);
      saveJSON(QBOX_KEY, next);
      return next;
    });
  }, []);
  ...
  /* ✓ 확인하면 메세지함에서 사라져요 */
  const doneMsg = useCallback((kind, id) => setMsgBox((v) => {
    const next = { ...v, [kind]: (v[kind] || []).filter((m) => m.id !== id) };
    saveJSON(MSG_KEY, next); return next;
  }), []);
  const doneMsgAll = useCallback((kind) => {
    if (kind === "notice") { readAll("notice"); return; }
    setMsgBox((v) => { const next = { ...v, [kind]: [] }; saveJSON(MSG_KEY, next); return next; });
  }, [readAll]);
  const doneNotice = useCallback((id) => setNoticeSeen((prev) => {
    const next = { ...prev, [id]: true }; saveJSON("echotown_noticeseen_ids", next); return next;
  }), []);
  ...
  const unreadMsgCount = (msgBox.invite || []).length + (msgBox.gift || []).length
    + (msgBox.dm || []).length + (msgBox.call || []).length
  ...
  useEffect(() => {
    onChatRef.net = (kind, p) => {
      ...
      if (kind === "shr") {
        if (p.row) {
          setShrineItems((v) => (v.some((x) => x.id === p.row.id) ? v : [p.row, ...v].slice(0, 120)));
          if (p.row.reviewer === (myName || "") && p.row.who !== (myName || "")) {
            playBell();
      ...
      if (kind === "qparty") {
        if (p.who !== (myName || "나")) pushQuest({ kind: "join", who: p.who, title: p.title || "퀘스트", qid: p.qid, mapId: p.mapId });
        setQAccept((a) => {
      ...
      if (kind === "qstart") {
        if (p.who !== (myName || "나")) pushQuest({ kind: "start", who: p.who, title: p.title || "퀘스트", qid: p.qid, mapId: p.mapId });
        setQAccept((a) => {
          const cur = a[p.qid] || { party: [], agree: [], locked: false, started: false, title: p.title || "" };
      ...
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        {view === "big" && bigMeta && (bigMeta.id === "alba" ? <AlbaView onBack={backToWorld} /> : <BigBuildingView b={bigMeta} qs={qs} day={day} onRun={runQuest} onBack={backToWorld} />)}        {view === "house" && houseMeta && (unlocked[houseId] ? (
          <HomeView isMine={isMyHouse(houseMeta.name)} myName={myName}
            ...
            layout={isMyHouse(houseMeta.name) ? homeLayout : ((houseDecor[houseId] || {}).layout || {})}
            ...
            fishes={isMyHouse(houseMeta.name) ? fishes : ((houseDecor[houseId] || {}).fishes || [])}
            hasAquarium={isMyHouse(houseMeta.name) ? facilities.includes("aquarium") : !!(houseDecor[houseId] || {}).aqua}
            hasYard={isMyHouse(houseMeta.name) ? facilities.includes("yard") : !!(houseDecor[houseId] || {}).yard}
            petsAtHome={isMyHouse(houseMeta.name) ? PETS.filter((x) => pets.includes(x.id) && x.id !== activePet).map((x) => `${x.emoji} ${x.name}`) : ((houseDecor[houseId] || {}).pets || [])}
            onOpenAqua={() => setAquaOpen(true)} onOpenYard={() => setYardOpen(true)}
            gifts={isMyHouse(houseMeta.name) ? homeGifts : ((houseDecor[houseId] || {}).gifts || [])}
            fridge={isMyHouse(houseMeta.name) ? fridge : ((houseDecor[houseId] || {}).fridge || [])}
            house={houseMeta}
            skin={isMyHouse(houseMeta.name) ? houseSkin : ((IKEA_ITEMS.house || []).find((x) => x.id === (houseDecor[houseId] || {}).skin) || null)}
            extras={isMyHouse(houseMeta.name) ? myFurni : ((houseDecor[houseId] || {}).extras || [])}
            onBack={backToWorld} bubble={bubble} />
        ) : (
        ...
        {view === "sea" && <SeaView pos={seaPos} setPos={setSeaPos} others={netOthers} bubble={bubble}
        ...
        {view === "project" && <BossMapView myName={myName} onBack={backToWorld} onGoSchool={(id) => setView(id)} onClearQuest={(isBoss, mode, title) => { bump(isBoss ? "boss" : "quest"); if (!isBoss && mode === "hard") learnSkill(title); }}
          ...
          onStart={(qid, mapId) => { setQAccept((a) => ({ ...a, [qid]: { ...a[qid], started: true } })); if (netSendEvent) netSendEvent("qstart", { qid, who: myName || "나", title: (qAccept[qid] || {}).title, mapId }); pushQuest({ kind: "start", who: myName || "나", title: (qAccept[qid] || {}).title || "퀘스트", qid, read: true }); showNotice("▶ 퀘스트를 시작했어요!"); }}
...
{view === "videoschool" && <SchoolView school={view} onBack={backToWorld} cleared={schoolDone} onClear={clearSchool} />}
      ...
      {shopOpen && <FisherShop onClose={() => setShopOpen(false)} gold={gold} bait={bait} rod={rod} bag={catchBag}
        onBuyBait={(n, p) => { if (gold < p) return; setGold((g) => g - p); setBait((b) => b + n); showNotice(`🪱 지렁이 ${n}개를 샀어요`); }}
        onBuyRod={(r) => { if (gold < r.price || rod >= r.id) return; setGold((g) => g - r.price); setRod(r.id); showNotice(`${r.emoji} ${r.name}을 샀어요!`); }}
        onSell={sellOne} onSellAll={sellAll} onOpenBox={openBox} />}
      {dexOpen && <SeaDex rod={rod} caught={caughtDex} onClose={() => setDexOpen(false)} />}
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 330 }}>
            <div style={{ background: C.parch, border: `4px solid ${C.ink}`, borderRadius: 14, padding: 20, boxShadow: "0 10px 26px rgba(0,0,0,0.5)" }}>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
                ...
                <PxButton tone="gold" onClick={() => { setStartPop(null); setHubTab(startPop && startPop.nShr > 0 ? "quest" : "quest"); setHubOpen(true); }} style={{ flex: 1.4, padding: 11, fontSize: 13 }}>🗺 퀘스트함 열기</PxButton>
              </div>
            </div>
          </div>
        </div>
      ...
      {hubOpen && (() => {
        const goQuest = (q) => {
          setHubOpen(false);
          if (q && q.goto === "shrine") { setView("questdone"); return; }
          if (q && q.id) setQuestJump({ mapId: q.mapId, qid: q.id });
          setView("project");
        };
        const tabs = [
          { k: "home", label: "🏠 요약" },
          { k: "quest", label: "🗺 퀘스트", n: unreadQuestCount },
          { k: "msg", label: "✉️ 메세지", n: unreadMsgCount },
        ];
        // 오늘 집계
        const today = new Date().toLocaleDateString("sv-SE");
        const isToday = (at) => (at || "").length > 0;   // 문자열 날짜라 대략 표시용
        const feed = [
          ...questBox.slice(0, 6).map((r) => ({ t: r.at, ic: QBOX_ICON[r.kind] || "🔔", who: r.who, txt: r.title, tab: "quest" })),
        ];
        const mapPct = (() => {
          let done = 0, all = 0;
          (bossMaps || []).forEach((m) => {
        ...
        return (
          <Sheet icon="🔔" title="알림함" onClose={() => setHubOpen(false)} tab={hubTab} setTab={setHubTab} tabs={tabs} maxW={480}>
            {hubTab === "home" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {/* 진행도 */}
                <div style={{ background: C.white, border: `3px solid ${C.ink}`, borderRadius: 10, padding: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 8 }}><span style={{ fontSize: 17 }}>🗺</span><b style={{ flex: 1, fontSize: 13.5 }}>보스맵 진행도</b><b style={{ fontSize: 13, color: C.good }}>{mapPct}%</b></div>
                  <div style={{ height: 14, borderRadius: 8, background: "#eadfc6", border: `2px solid ${C.ink}`, overflow: "hidden" }}>
                    <div style={{ width: mapPct + "%", height: "100%", background: "linear-gradient(90deg,#5fc99a,#2f9e6e)" }} />
                  </div>
                </div>
                {/* 오늘의 정산 */}
                <div style={{ background: C.white, border: `3px solid ${C.ink}`, borderRadius: 10, padding: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 8 }}><span style={{ fontSize: 17 }}>📊</span><b style={{ flex: 1, fontSize: 13.5 }}>오늘의 정산</b></div>
                  <div style={{ display: "flex", gap: 8 }}>
                    {[["🆕 신규", questBox.filter((r) => r.kind === "new").length], ["✦ 파편", questBox.filter((r) => r.kind === "shrine").length], ["🎁 받을 보상", questBox.filter((r) => r.kind === "reward").length], ["✉️ 메세지", unreadMsgCount]].map(([lb, n]) => (
                      <div key={lb} style={{ flex: 1, textAlign: "center", background: C.parch, border: `2px solid ${C.parchEdge}`, borderRadius: 8, padding: "9px 4px" }}>
                        <div style={{ fontSize: 18, fontWeight: "bold", color: n > 0 ? C.danger : C.inkSoft }}>{n}</div>
                        <div style={{ fontSize: 9.5, color: C.inkSoft, marginTop: 2 }}>{lb}</div>
                      </div>
                    ))}
                  </div>
                </div>
                {/* 최근 소식 */}
                <div style={{ background: C.white, border: `3px solid ${C.ink}`, borderRadius: 10, padding: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 8 }}><span style={{ fontSize: 17 }}>📰</span><b style={{ flex: 1, fontSize: 13.5 }}>최근 소식</b><PxButton tone="wood" onClick={() => setHubTab("quest")} style={{ fontSize: 10, padding: "4px 8px" }}>전체 보기</PxButton></div>
            ...
            {hubTab === "quest" && (
              <QuestBoxSheet embed onClose={() => setHubOpen(false)} rows={questBox} onRead={readQuestBox} onClear={clearQuestBox} onDoneRow={doneQuestRow} onClaim={claimReward}
                maps={bossMaps} accept={qAccept} cleared={bossCleared} myName={myName} onGo={goQuest} />
            )}
            {hubTab === "msg" && (
              <MessageCenter embed onClose={() => setHubOpen(false)} myName={myName} box={msgBox} notices={allNotices} onReadAll={readAll}
                onDone={doneMsg} onDoneAll={doneMsgAll} onDoneNotice={doneNotice}
                onAnswerInvite={(m, ok) => {
                  let reason = "";
                  if (!ok) { reason = (window.prompt(`${m.from}님에게 보낼 불참 사유를 적어주세요`, "") || "").trim(); if (!reason) return; }
                ...
                onGoMeeting={(m) => { setHubOpen(false); setMeetingId(m.roomId || "m1"); setView("meeting"); }}
                onOpenDm={(name) => { setHubOpen(false); setDmWith({ name, avatar: "🧑" }); }}
                onCallBack={(name) => { setHubOpen(false); setCallWith({ name, avatar: "🧑" }); if (netSendEvent) netSendEvent("call", { to: name, from: myName || "나" }); }} />
            )}
          </Sheet>
        );
      })()}
  ...
  return (
    <style>{`
      * { -webkit-tap-highlight-color: transparent; }
      ...
      @keyframes seaWave { from { background-position: 0 0; } to { background-position: 62px 0; } }
      .sea-wave { animation: seaWave 3.4s linear infinite; }
      @keyframes seaWave2 { from { background-position: 0 0; } to { background-position: -78px 0; } }
      .sea-wave2 { animation: seaWave2 5.2s linear infinite; }
      @keyframes seaFloat { 0%,100% { transform: translateY(0) rotate(-3deg); } 50% { transform: translateY(-7px) rotate(3deg); } }
      .sea-float { animation: seaFloat 3.6s ease-in-out infinite; }
