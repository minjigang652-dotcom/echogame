...
function CafeRoom({ crawledLinks = [], nickname = '', onBack }) {
  ...
  const [exposts, setExposts] = useState([]);   // 📚 예시 게시판
  const [exOpen, setExOpen] = useState(false); const [exWrite, setExWrite] = useState(false);
  const [exTitle, setExTitle] = useState(''); const [exBody, setExBody] = useState(''); const [exView, setExView] = useState(null);
  const [exImgs, setExImgs] = useState([]);   // 예시 글 사진
  ...
  return (
    <>
      <div className="nsp-roomrow">
        <button className="nsp-roomback" onClick={() => onBack && onBack()}>← 방 목록으로</button>
          ...
          <div className="nsp-modal" onClick={(e) => e.stopPropagation()}>
            {exView ? (
              <>
                ...
                {(exView.images || []).length > 0 && (
                  <div className="ex-viewimgs">
                    {(exView.images || []).map((im, i) => (
                      <a key={i} href={im.url} download={im.name || ('image' + i + '.jpg')}><img src={im.url} alt="" /></a>
                    ))}
                  </div>
                )}
              ...
              <>
                {!admin ? (
                  ...
                  <>
                    ...
                    <div className="ex-imgadd">
                      <label className="ex-imgbtn">🖼 사진 추가
                        <input type="file" accept="image/*" multiple hidden onChange={async (e) => {
                          const files = Array.from(e.target.files || []); e.target.value = '';
                          for (const f of files) { try { const url = await compressImage(f, 1200, 0.75, 'image/jpeg'); setExImgs((v) => [...v, { url, name: f.name }].slice(0, 8)); } catch (err) {} }
                        }} />
                      </label>
                      <div className="ex-thumbs">
                        {exImgs.map((im, i) => (
                          <div key={i} className="ex-thumb"><img src={im.url} alt="" /><button onClick={() => setExImgs((v) => v.filter((_, x) => x !== i))}>✕</button></div>
                        ))}
                      </div>
                    </div>
                    <div className="nsp-modal-btns">
                      <button className="nsp-btn ghost" onClick={() => { setExWrite(false); setExTitle(''); setExBody(''); setExImgs([]); }}>취소</button>
                      <button className="nsp-btn" onClick={() => {
                        if (!exTitle.trim() || !exBody.trim()) return;
                        saveExposts([{ id: 'e' + Date.now(), title: exTitle.trim(), body: exBody.trim(), images: exImgs, at: nowISO(), by: nickname || '관리자' }, ...exposts].slice(0, 200));
                        setExTitle(''); setExBody(''); setExImgs([]); setExWrite(false);
                      }}>등록</button>
                    </div>
                  </>
                )}
                ...
                <div className="kin-ex-list">
                  ...
                  {exposts.map((p) => (
                    <button key={p.id} className="kin-ex-item" onClick={() => setExView(p)}>
                      ...
                      <span className="kin-ex-sub">✍️ {p.by} · {(p.at || '').slice(0, 10)}{(p.images || []).length > 0 ? ` · 🖼 ${p.images.length}` : ''}</span>
...
function KinRoom({ crawledLinks = [], nickname = '', onBack, pfx = 'kin' }) {
...
/* ======================= 📊 지식인 상위 방 (키워드 네모칸 + 답변불가/완료) ======================= */
function KinTopRoom({ crawledLinks = [], nickname = '', onBack }) {
  const PFX = 'kintop';
  const [products, setProducts] = useState([]);
  const [state, setState] = useState({ links: {} });   // { url: { status:'done'|'fail', at, by, keyword } }
  ...
  // 키워드별 링크: 답변완료된 링크는 네모칸에서 제외 (중복 크롤링 방지)
  const linksByKeyword = {};
  curKeywords.forEach((kw) => { linksByKeyword[kw.word] = []; });
  crawledLinks.forEach((lk) => {
    const w = (lk.keyword || '').trim();
    const cur = state.links[lk.url];
    if (w in linksByKeyword && !(cur && cur.status === 'done')) linksByKeyword[w].push(lk);
  });
  const totalLinks = Object.values(linksByKeyword).reduce((a, arr) => a + arr.length, 0);
  const elapsedMin = timer.phase === 'running' ? Math.floor((Date.now() - timer.start) / 60000) : 0;
            ...
            <div key={kw.id} className="cafe-kwbox">
              ...
              <div className="cafe-links">
                ...
                {(linksByKeyword[kw.word] || []).map((lk, i) => {
                  const cur = state.links[lk.url];
                  return (
                    <div key={i} className="kt-linkrow">
                      <a className="kin-link" href={lk.url} target="_blank" rel="noreferrer">{lk.title || lk.url}</a>
                      <div className="kin-btns">
                        <button className={`kin-b fail ${cur && cur.status === 'fail' ? 'on' : ''}`} onClick={() => mark(lk, 'fail')}>답변불가</button>
                        <button className={`kin-b done ${cur && cur.status === 'done' ? 'on' : ''}`} onClick={() => mark(lk, 'done')}>답변완료</button>
                      </div>
...
function NaverSchoolPanel({ open, onClose, nickname: nicknameProp }) {
  ...
  const [data, setData] = useState({ keywords: [], urls: [], cafe: [], kin: [], yt: [], kintop: [], cafeLinks: [] });
  ...
  async function load() {
    ...
    try {
      ...
      const [kw, urls, cafe, kin, yt, kintop, clinks] = await Promise.all([
        ...
        j(`${MOIP_API}/api/echo/posts?source=kintop`).catch(() => ({ items: [] })),
        j(`${MOIP_API}/api/echo/cafe-links`),
      ]);
      const pick = (x) => x.items || x;
      setData({ keywords: pick(kw), urls: pick(urls), cafe: pick(cafe), kin: pick(kin), yt: pick(yt), kintop: pick(kintop), cafeLinks: pick(clinks) });
    ...
    <div className="nsp-overlay" onClick={onClose}>
      ...
      <div className="nsp-panel" onClick={(e) => e.stopPropagation()}>
        <div className="nsp-hd">
          ...
          <button type="button" onClick={() => setChatOpen(true)} title="코코 상담소 — 궁금한 걸 물어보세요" style={{ marginLeft: "auto", cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: "bold", background: "#fff", color: "#2f6b25", border: "2px solid #2f6b25", borderRadius: 8, padding: "5px 11px" }}>🐣 코코</button>
        ...
        <div className="nsp-bd">
          ...
          {tab !== 'cafe' && tab !== 'kin' && tab !== 'yt' && tab !== 'kinTop' && (
          <div className="nsp-roomrow">
          ...
          {/* 빈 방: 유튜브 최신글 */}
          {tab === 'yt' && (
            <KinRoom crawledLinks={data.yt || []} nickname={nicknameProp || nickname} onBack={() => setTab(null)} pfx="yt" />
          )}
          {/* ① 카페 외부 (네이버 키워드) */}
          {tab === 'kw' && (() => {
          ...
          {tab === 'cafe' && (
            <CafeRoom crawledLinks={data.cafe || []} nickname={nicknameProp || nickname} onBack={() => setTab(null)} />
          )}

          {/* ④ 지식인 최신글 (답변 카운터 + 1시간 50개 타이머) */}
          {tab === 'kin' && <KinRoom crawledLinks={data.kin || []} nickname={nicknameProp || nickname} onBack={() => setTab(null)} />}
...
.cafe-prods{display:flex;flex-wrap:wrap;gap:6px;flex:1;margin-bottom:10px;}
...
.kt-linkrow{display:flex;flex-direction:column;gap:4px;padding:6px;background:#f7f3e8;border:1px solid #d9c79b;border-radius:6px;}
.kt-linkrow .kin-link{font-size:11.5px;}
.kt-linkrow .kin-btns{justify-content:flex-end;}
.kt-linkrow .kin-b{padding:3px 8px;font-size:10px;}
...
.ex-imgadd{margin-top:8px;}
.ex-imgbtn{display:inline-block;background:#efe0bd;border:2px solid #4a3a22;border-radius:8px;padding:7px 12px;cursor:pointer;font-family:'DotGothic16',monospace;font-size:12px;font-weight:bold;color:#4a3a22;}
.ex-thumbs{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;}
.ex-thumb{position:relative;width:70px;height:70px;border:2px solid #4a3a22;border-radius:6px;overflow:hidden;}
.ex-thumb img{width:100%;height:100%;object-fit:cover;}
.ex-thumb button{position:absolute;top:-2px;right:-2px;background:#c0563a;color:#fff;border:1px solid #4a3a22;border-radius:5px;width:18px;height:18px;cursor:pointer;font-size:10px;padding:0;}
.ex-viewimgs{display:flex;flex-wrap:wrap;gap:8px;margin-top:10px;}
.ex-viewimgs a{display:block;border:2px solid #4a3a22;border-radius:8px;overflow:hidden;max-width:100%;}
.ex-viewimgs img{max-width:100%;max-height:300px;display:block;}
